...
ORDER BY
	...
	for rows.Next() {
		...
		col.isNullable, _ = nullable.Unpack(ColTypeBool).(bool)
		col.isIdentity, _ = ident.Unpack(ColTypeBool).(bool)
		if descr.Valid {
			col.comment = descr.String
		}

		if s, _ := col.defaultValue.Unpack(ColTypeString).(string); strings.Contains(s, "nextval") {
			col.isIdentity = true
			col.isAutoIncrement = true
		} else if sid, _ := identGen.Unpack(ColTypeString).(string); sid != "" {
			if sid == "ALWAYS" {
