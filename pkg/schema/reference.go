...
type Reference struct {
	...
	*/
	...
	// foreignKey is the generated column that will contain a copy of the primary key in the other table
	foreignKey *Column
	// primaryKey is the primary key column in the other table.
	primaryKey *Column
}
...
func (r *Reference) infer(db *Database, table *Table) error {
	...
	if t := db.FindTable(r.Table); t == nil {
		...
		r.foreignKey = &fk
		r.primaryKey = pk
		table.Columns = append(table.Columns, r.foreignKey)
	}
...
// ReferenceColumns returns the foreign key column that was generated by the info in the reference
// and the primary key column it refers to.
func (r *Reference) ReferenceColumns() (*Column, *Column) {
	return r.foreignKey, r.primaryKey
}
