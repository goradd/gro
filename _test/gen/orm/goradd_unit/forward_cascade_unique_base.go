...
func (o *forwardCascadeUniqueBase) SetName(name string) {
	o.nameIsValid = true

	if utf8.RuneCountInString(name) > ForwardCascadeUniqueNameMaxLength {
...
func (o *forwardCascadeUniqueBase) SetReverseID(i interface{}) {
	o.reverseIDIsValid = true
	if i == nil {
...
// The ForwardCascadeUniquesBuilder uses the QueryBuilderI interface from the database to build a query.
// All query operations go through this query builder.
// End a query by calling either Load, Count, or Delete
type ForwardCascadeUniquesBuilder struct {
	builder query.QueryBuilderI
}

func newForwardCascadeUniqueBuilder(ctx context.Context) *ForwardCascadeUniquesBuilder {
	b := &ForwardCascadeUniquesBuilder{
		builder: db.GetDatabase("goradd_unit").NewBuilder(ctx),
	}
	return b.Join(node.ForwardCascadeUnique())
}

// Load terminates the query builder, performs the query, and returns a slice of ForwardCascadeUnique objects. If there are
// any errors, they are returned in the context object. If no results come back from the query, it will return
// an empty slice
func (b *ForwardCascadeUniquesBuilder) Load() (forwardCascadeUniques []*ForwardCascadeUnique) {
...
// any errors, they are returned in the context object. If no results come back from the query, it will return
// an empty slice.
func (b *ForwardCascadeUniquesBuilder) LoadI() (forwardCascadeUniques []interface{}) {
	results := b.builder.Load()
	if results == nil {
		return
	}
...
func (b *ForwardCascadeUniquesBuilder) LoadCursor() forwardCascadeUniquesCursor {
...
func (b *ForwardCascadeUniquesBuilder) Get() *ForwardCascadeUnique {
	results := b.Load()
...
func (b *ForwardCascadeUniquesBuilder) Expand(node query.Expander) *ForwardCascadeUniquesBuilder {
	n := node.(query.NodeI)
	if query.NodeTableName(query.RootNode(n)) != "forward_cascade_unique" {
		panic("you can only expand a node that is rooted at node.ForwardCascadeUnique()")
	}
...
func (b *ForwardCascadeUniquesBuilder) Limit(maxRowCount int, offset int) *ForwardCascadeUniquesBuilder {
...
func (b *ForwardCascadeUniquesBuilder) Distinct() *ForwardCascadeUniquesBuilder {
	b.builder.Distinct()
	return b
}
...
func (b *ForwardCascadeUniquesBuilder) Delete() {
	b.builder.Delete()
	broadcast.BulkChange(b.builder.Context(), "goradd_unit", "forward_cascade_unique")
}

// Subquery uses the query builder to define a subquery within a larger query. You MUST include what
// you are selecting by adding Alias or Select functions on the subquery builder. Generally you would use
// this as a node to an Alias function on the surrounding query builder.
func (b *ForwardCascadeUniquesBuilder) Subquery() *query.SubqueryNode {
...
func CountForwardCascadeUniqueByID(ctx context.Context, id string) int {
	return int(queryForwardCascadeUniques(ctx).Where(op.Equal(node.ForwardCascadeUnique().ID(), id)).Count(false))
}
...
func CountForwardCascadeUniqueByName(ctx context.Context, name string) int {
	return int(queryForwardCascadeUniques(ctx).Where(op.Equal(node.ForwardCascadeUnique().Name(), name)).Count(false))
}
...
func CountForwardCascadeUniqueByReverseID(ctx context.Context, reverseID string) int {
	...
	return int(queryForwardCascadeUniques(ctx).Where(op.Equal(node.ForwardCascadeUnique().ReverseID(), reverseID)).Count(false))
}

// load is the private loader that transforms data coming from the database into a tree structure reflecting the relationships
// between the object chain requested by the user in the query.
// Care must be taken in the query, as Select clauses might not be honored if the child object has fields selected which the parent object does not have.
func (o *forwardCascadeUniqueBase) load(m map[string]interface{}, objThis *ForwardCascadeUnique, objParent interface{}, parentKey string) {
	...
	if v, ok := m["Reverse"]; ok {
		if objReverse, ok2 := v.(map[string]interface{}); ok2 {
			...
			o.objReverse.load(objReverse, o.objReverse, objThis, "ForwardCascadeUnique")
			o.reverseIDIsValid = true
			o.reverseIDIsDirty = false
		} else {
...
func (o *forwardCascadeUniqueBase) Save(ctx context.Context) {
	if o._restored {
		o.update(ctx)
	} else {
		o.insert(ctx)
...
func (o *forwardCascadeUniqueBase) update(ctx context.Context) {
	if !o._restored {
...
func (o *forwardCascadeUniqueBase) insert(ctx context.Context) {
	d := Database()
	db.ExecuteTransaction(ctx, d, func() {
...
func (o *forwardCascadeUniqueBase) Delete(ctx context.Context) {
	if !o._restored {
...
// deleteForwardCascadeUnique deletes the associated record from the database.
