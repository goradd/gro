...
// The RootUnBuilder uses the query.BuilderI interface to build a query.
// All query operations go through this query builder.
// End a query by calling either Load, LoadCursor, Get, Count, or Delete
type RootUnBuilder interface {
	// Join(alias string, joinedTable query.Node, condition query.Node) RootUnBuilder
...
func (o *rootUnBase) update(ctx context.Context) error {
	...
	err := db.ExecuteTransaction(ctx, d, func() error {
		...
		if o.revLeafUnIsDirty {
			...
			if obj, err := QueryLeafUns(ctx).
				...
				Get(); err != nil {
				return err
			} else if obj != nil && obj.PrimaryKey() != o.revLeafUn.PrimaryKey() {
	...
	o.resetDirtyStatus()
	if len(modifiedFields) != 0 {
		broadcast.Update(ctx, "goradd_unit", "root_un", o._originalPK, all.SortedKeys(modifiedFields)...)
	}
