...
type rootUnBase struct {
	...
	// Reverse references
	rootUnLeafUns        maps.SliceMap[string, *LeafUn] // Objects in the order they were queried
	rootUnLeafUnsIsDirty bool
...
const (
	...
	RootUnRootUnLeafUnField = `rootUnLeafUns`
)
...
func (o *rootUnBase) Initialize() {
	...
	o.rootUnLeafUns.Clear()
	o.rootUnLeafUnsIsDirty = false
...
func (o *rootUnBase) SetPrimaryKey(v string) {
	if o._restored {
		panic("error: Do not change a primary key for a record that has been saved. Instead, save a copy and delete the original.")
	}
	if utf8.RuneCountInString(v) > RootUnIDMaxLength {
		panic("attempted to set RootUn.Value to a value larger than its maximum length in runes")
	}
	o.idIsLoaded = true
	o.idIsDirty = true
	o.id = v
...
// RootUnLeafUn returns a single LeafUn object by primary key, if one was loaded.
// Otherwise, it will return nil. It will not return LeafUn objects that are not saved.
func (o *rootUnBase) RootUnLeafUn(pk string) *LeafUn {
	v := o.rootUnLeafUns.Get(pk)
	return v
}

// RootUnLeafUns returns a slice of LeafUn objects if loaded.
func (o *rootUnBase) RootUnLeafUns() []*LeafUn {
	return o.rootUnLeafUns.Values()
}

// LoadRootUnLeafUns loads a new slice of LeafUn objects and returns it.
func (o *rootUnBase) LoadRootUnLeafUns(ctx context.Context) ([]*LeafUn, error) {
	if o.IsNew() {
		return nil, nil
	}
	for obj := range o.rootUnLeafUns.ValuesIter() {
		if obj.IsDirty() {
			panic("You cannot load over items that have changed but have not been saved.")
		}
	}

	objs, err := LoadLeafUnsByRootUnID(ctx, o.PrimaryKey())
	if err != nil {
		return nil, err
	}
	o.rootUnLeafUns.Clear()
	...
	for _, obj := range objs {
		pk := obj.ID()
		o.rootUnLeafUns.Set(pk, obj)
	}

	if o.rootUnLeafUns.Len() == 0 {
		return nil, nil
	}
	return o.rootUnLeafUns.Values(), nil
}

// CountRootUnLeafUns does a database query and returns the number of LeafUn
// objects currently in the database that have a RootUnID value that equals this objects primary key.
func (o *rootUnBase) CountRootUnLeafUns(ctx context.Context) (int, error) {
	return CountLeafUnsByRootUnID(ctx, o.PrimaryKey())
}

// SetRootUnLeafUns associates the objects in objs with this RootUn by setting
// their RootUnID values to this object's primary key.
// WARNING! If it has RootUnLeafUns already associated with it that will not be associated after a save,
// Save will panic. Be sure to delete those RootUnLeafUns or otherwise fix those pointers before calling save.
func (o *rootUnBase) SetRootUnLeafUns(objs ...*LeafUn) {
	for obj := range o.rootUnLeafUns.ValuesIter() {
		if obj.IsDirty() {
	...
	if v, ok := m["RootUnLeafUn"]; ok {
		...
		case []map[string]any: // array expansion
			...
			for _, v3 := range v2 {
				obj := new(LeafUn)
				obj.unpack(v3, obj)
				o.rootUnLeafUns.Set(obj.PrimaryKey(), obj)
			}
		default:
			panic("Wrong type found for rootUnLeafUns object.")
		}
	} else {
		o.rootUnLeafUns.Clear()
		o.rootUnLeafUnsIsDirty = false
...
func (o *rootUnBase) update(ctx context.Context) error {
	...
	err := db.WithTransaction(ctx, d, func(ctx context.Context) error {
		...
		if o.rootUnLeafUnsIsDirty {
			...
			if oldObjs, err := QueryLeafUns(ctx).
				...
				for _, obj := range oldObjs {
					if !o.rootUnLeafUns.Has(obj.PrimaryKey()) {
				...
				keys := o.rootUnLeafUns.Keys() // Make a copy of the keys, since we will change the slicemap while iterating
				for i, k := range keys {
					obj := o.rootUnLeafUns.Get(k)
					if obj == nil {
						// object was deleted during save?
						continue
					}
					obj.SetRootUnID(o.PrimaryKey())
					obj.rootUnIDIsDirty = true // force a change in case data is stale
					if err = obj.Save(ctx); err != nil {
						return err
					}
					if obj.PrimaryKey() != k {
						// update slice map key without changing order
						o.rootUnLeafUns.Delete(k)
						o.rootUnLeafUns.SetAt(i, obj.PrimaryKey(), obj)
					}
				}
			}
...
func (o *rootUnBase) insert(ctx context.Context) (err error) {
	...
	err = db.WithTransaction(ctx, d, func(context.Context) error {
		...
		if o.rootUnLeafUns.Len() > 0 {
			keys := o.rootUnLeafUns.Keys()
			for i, k := range keys {
				obj := o.rootUnLeafUns.Get(k)
				obj.SetRootUnID(newPK)
				if err = obj.Save(ctx); err != nil {
					return err
				}
				if obj.PrimaryKey() != k {
...
//
// Associated RootUnLeafUn will also be deleted since their RootUn fields are not nullable.
	...
	err = db.WithTransaction(ctx, d, func(context.Context) error {

		{
			objs, err := QueryLeafUns(ctx).
				Where(op.Equal(node.LeafUn().RootUn(), o.id)).
				Load()
			if err != nil {
				return err
			}
...
func (o *rootUnBase) encodeTo(enc db.Encoder) error {
	...
	if err := enc.Encode(&o.rootUnLeafUns); err != nil {
		return err
	}

	if err := enc.Encode(o.rootUnLeafUnsIsDirty); err != nil {
		return err
	}
...
func (o *rootUnBase) decodeFrom(dec db.Decoder) (err error) {
	...
	if err = dec.Decode(&o.rootUnLeafUns); err != nil {
		return fmt.Errorf("error decoding RootUn.rootUnLeafUns: %w", err)
	}

	if err = dec.Decode(&o.rootUnLeafUnsIsDirty); err != nil {
		return fmt.Errorf("error decoding RootUn.rootUnLeafUnsIsDirty: %w", err)
	}
