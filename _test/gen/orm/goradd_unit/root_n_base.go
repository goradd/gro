...
type rootNBase struct {
	...
	// Reverse references
	rootNLeafNs        maps.SliceMap[string, *LeafN] // Objects in the order they were queried
	rootNLeafNsIsDirty bool
...
const (
	RootNIDField         = `id`
	RootNNameField       = `name`
	RootNRootNLeafNField = `rootNLeafNs`
)
...
func (o *rootNBase) SetPrimaryKey(v string) {
	...
	if utf8.RuneCountInString(v) > RootNIDMaxLength {
		panic("attempted to set RootN.Value to a value larger than its maximum length in runes")
	}
	o.idIsLoaded = true
	o.idIsDirty = true
	o.id = v
...
// RootNLeafN returns a single LeafN object by primary key, if one was loaded.
// Otherwise, it will return nil. It will not return LeafN objects that are not saved.
func (o *rootNBase) RootNLeafN(pk string) *LeafN {
	v := o.rootNLeafNs.Get(pk)
	return v
}

// RootNLeafNs returns a slice of LeafN objects if loaded.
func (o *rootNBase) RootNLeafNs() []*LeafN {
	return o.rootNLeafNs.Values()
}

// LoadRootNLeafNs loads a new slice of LeafN objects and returns it.
func (o *rootNBase) LoadRootNLeafNs(ctx context.Context) ([]*LeafN, error) {
	if o.IsNew() {
		return nil, nil
	}
	for obj := range o.rootNLeafNs.ValuesIter() {
		if obj.IsDirty() {
	...
	for _, obj := range objs {
		pk := obj.ID()
		o.rootNLeafNs.Set(pk, obj)
	}

	if o.rootNLeafNs.Len() == 0 {
		return nil, nil
	}
	return o.rootNLeafNs.Values(), nil
}

// CountRootNLeafNs does a database query and returns the number of LeafN
// objects currently in the database that have a RootNID value that equals this objects primary key.
func (o *rootNBase) CountRootNLeafNs(ctx context.Context) (int, error) {
	return CountLeafNsByRootNID(ctx, o.PrimaryKey())
}

// SetRootNLeafNs associates the objects in objs with this RootN by setting
// their RootNID values to this object's primary key.
// WARNING! If it has RootNLeafNs already associated with it that will not be associated after a save,
// Save will panic. Be sure to delete those RootNLeafNs or otherwise fix those pointers before calling save.
func (o *rootNBase) SetRootNLeafNs(objs ...*LeafN) {
	for obj := range o.rootNLeafNs.ValuesIter() {
		if obj.IsDirty() {
	...
	if v, ok := m["RootNLeafN"]; ok {
		...
		case []map[string]any: // array expansion
			...
			for _, v3 := range v2 {
				obj := new(LeafN)
				obj.unpack(v3, obj)
				o.rootNLeafNs.Set(obj.PrimaryKey(), obj)
			}
		default:
			panic("Wrong type found for rootNLeafNs object.")
		}
	} else {
		o.rootNLeafNs.Clear()
		o.rootNLeafNsIsDirty = false
		...
		if o.rootNLeafNsIsDirty {
			...
			if oldObjs, err := QueryLeafNs(ctx).
				...
				for _, obj := range oldObjs {
					if !o.rootNLeafNs.Has(obj.PrimaryKey()) {
				...
				keys := o.rootNLeafNs.Keys() // Make a copy of the keys, since we will change the slicemap while iterating
				for i, k := range keys {
					obj := o.rootNLeafNs.Get(k)
					if obj == nil {
					...
					if obj.PrimaryKey() != k {
						...
						o.rootNLeafNs.Delete(k)
						o.rootNLeafNs.SetAt(i, obj.PrimaryKey(), obj)
					}
				}
			}
...
//
// Associated RootNLeafN will also be deleted since their RootN fields are not nullable.
...
func (o *rootNBase) encodeTo(enc db.Encoder) error {
	...
	if err := enc.Encode(&o.rootNLeafNs); err != nil {
		return err
	}

	if err := enc.Encode(o.rootNLeafNsIsDirty); err != nil {
		return err
	}
...
func (o *rootNBase) decodeFrom(dec db.Decoder) (err error) {
	...
	if err = dec.Decode(&o.rootNLeafNs); err != nil {
		return fmt.Errorf("error decoding RootN.rootNLeafNs: %w", err)
	}

	if err = dec.Decode(&o.rootNLeafNsIsDirty); err != nil {
		return fmt.Errorf("error decoding RootN.rootNLeafNsIsDirty: %w", err)
	}
