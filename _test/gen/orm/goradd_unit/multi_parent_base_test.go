...
func TestMultiParent_ReferenceLoad(t *testing.T) {
	...
	objPkOnly, err2 := LoadMultiParent(ctx, obj.PrimaryKey(),
		node.MultiParent().ID())
	...
	assert.Nil(t, obj2.Parent1MultiParents(), "Parent1MultiParent is not loaded initially")
	...
	assert.Nil(t, obj2.Parent2MultiParents(), "Parent2MultiParent is not loaded initially")
...
func TestMultiParent_Getters(t *testing.T) {
	...
	assert.Equal(t, obj.ID(), obj.Get(MultiParentIDField))
	assert.Equal(t, obj.Name(), obj.Get(MultiParentNameField))
	assert.Panics(t, func() { obj2.Name() })
	assert.Nil(t, obj2.Get(MultiParentNameField))
	// Not loaded
	assert.Nil(t, obj2.Parent1())
	assert.Nil(t, obj2.Get(MultiParentParent1Field))
	assert.Panics(t, func() { obj2.Parent1ID() })
	assert.Nil(t, obj2.Get(MultiParentParent1IDField))
	// Not loaded
	assert.Nil(t, obj2.Parent2())
	assert.Nil(t, obj2.Get(MultiParentParent2Field))
	assert.Panics(t, func() { obj2.Parent2ID() })
	assert.Nil(t, obj2.Get(MultiParentParent2IDField))
...
func TestMultiParent_QueryLoad(t *testing.T) {
	...
	objs, err := QueryMultiParents(ctx).
		Where(op.Equal(node.MultiParent().ID(), obj.ID())).
		OrderBy(node.MultiParent().ID()). // exercise order by
		Limit(1, 0).                      // exercise limit
		Calculation(node.MultiParent(), "IsTrue", op.Equal("A", "A")).
		Load()
	assert.NoError(t, err)
...
func TestMultiParent_QueryLoadI(t *testing.T) {
	...
	assert.Equal(t, obj.PrimaryKey(), objs[0].(*MultiParent).PrimaryKey())
}
