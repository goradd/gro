...
func TestMultiParent_InsertPanics(t *testing.T) {
	...
	obj.parent1IDIsLoaded = false
	assert.Panics(t, func() { obj.Save(ctx) })
	obj.parent1IDIsLoaded = true

	obj.parent2IDIsLoaded = false
	assert.Panics(t, func() { obj.Save(ctx) })
	obj.parent2IDIsLoaded = true
...
func TestMultiParent_ReferenceLoad(t *testing.T) {
	...
	// test eager loading
	obj3, _ := LoadMultiParent(ctx, obj.PrimaryKey(), node.MultiParent().ID(),
		node.MultiParent().Name(),
...
func TestMultiParent_ReferenceUpdateOldObjects(t *testing.T) {
	...
	obj2, _ := LoadMultiParent(ctx, obj.PrimaryKey(), node.MultiParent().ID(),
		node.MultiParent().Name(),
		node.MultiParent().Parent1MultiParents(),
		node.MultiParent().Parent2MultiParents(),
	)
