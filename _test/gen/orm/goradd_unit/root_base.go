...
type rootBase struct {
	...
	revLeafs        maps.SliceMap[string, *Leaf] // Objects in the order they were queried
	revLeafsIsDirty bool
...
// Leaf returns a single Leaf object by primary key, if one was loaded.
// Otherwise, it will return nil. It will not return Leaf objects that are not saved.
func (o *rootBase) Leaf(pk string) *Leaf {
	v := o.revLeafs.Get(pk)
	return v
}

// Leafs returns a slice of Leaf objects if loaded.
func (o *rootBase) Leafs() []*Leaf {
	return o.revLeafs.Values()
}

// LoadLeafs loads a new slice of Leaf objects and returns it.
func (o *rootBase) LoadLeafs(ctx context.Context, conditions ...interface{}) ([]*Leaf, error) {
	if o.IsNew() {
		return nil, nil
	}
	for obj := range o.revLeafs.ValuesIter() {
		if obj.IsDirty() {
	...
	qb := queryLeafs(ctx)
	cond := op.Equal(node.Leaf().RootID(), o.PrimaryKey())
	if conditions != nil {
	...
	for _, obj := range objs {
		pk := obj.ID()
		o.revLeafs.Set(pk, obj)
	}

	if o.revLeafs.Len() == 0 {
		return nil, nil
	}
	return o.revLeafs.Values(), nil
}

// CountLeafs does a database query and returns the number of Leaf
// objects currently in the database connected to this object.
func (o *rootBase) CountLeafs(ctx context.Context) (int, error) {
	return CountLeafsByRootID(ctx, o.PrimaryKey())
}

// SetLeafs associates the objects in objs with the Root.
// WARNING! If it has Leafs already associated with it that will not be associated after a save,
// Save will panic. Be sure to delete those Leafs or otherwise fix those pointers before calling save.
func (o *rootBase) SetLeafs(objs ...*Leaf) {
	for obj := range o.revLeafs.ValuesIter() {
		if obj.IsDirty() {
...
// Start a query by calling QueryRoots, which will select all
...
func (o *rootBase) load(m map[string]interface{}, objThis *Root) {
	...
	if v, ok := m["Leafs"]; ok {
		...
		default:
			panic("Wrong type found for revLeafs object.")
		}
	} else {
		o.revLeafs.Clear()
		o.revLeafsIsDirty = false
	...
	err := db.ExecuteTransaction(ctx, d, func() error {
		...
		if o.revLeafsIsDirty {
			...
			if oldObjs, err := QueryLeafs(ctx).
				Where(op.Equal(node.Leaf().RootID(), o.PrimaryKey())).
				Select(node.Leaf().RootID()).
				Load(); err != nil {
				return err
			} else {
				for _, obj := range oldObjs {
					if !o.revLeafs.Has(obj.PrimaryKey()) {
				...
				keys := o.revLeafs.Keys() // Make a copy of the keys, since we will change the slicemap while iterating
				for i, k := range keys {
					obj := o.revLeafs.Get(k)
					if obj == nil {
					...
					if obj.PrimaryKey() != k {
						...
						o.revLeafs.Delete(k)
						o.revLeafs.SetAt(i, obj.PrimaryKey(), obj)
					}
				}
			}
...
//
// Associated Leafs will also be deleted since their RootID fields are not nullable.
...
func (o *rootBase) encodeTo(enc db.Encoder) error {
	...
	if err := enc.Encode(&o.revLeafs); err != nil {
		return err
	}

	if err := enc.Encode(o.revLeafsIsDirty); err != nil {
		return err
	}
...
func (o *rootBase) decodeFrom(dec db.Decoder) (err error) {
	...
	if err = dec.Decode(&o.revLeafs); err != nil {
		return fmt.Errorf("error decoding Root.revLeafs: %w", err)
	}

	if err = dec.Decode(&o.revLeafsIsDirty); err != nil {
		return fmt.Errorf("error decoding Root.revLeafsIsDirty: %w", err)
	}
