...
type rootBase struct {
	...
	// Reverse references
	leafs        maps.SliceMap[string, *Leaf] // Objects in the order they were queried
	leafsIsDirty bool
...
func (o *rootBase) LoadLeafs(ctx context.Context) ([]*Leaf, error) {
	...
	for obj := range o.leafs.ValuesIter() {
		if obj.IsDirty() {
...
// SetLeafs associates the objects in objs with this Root by setting
...
func LoadRoot(ctx context.Context, pk string, selectNodes ...query.Node) (*Root, error) {
	return queryRoots(ctx).
		Where(op.Equal(node.Root().ID(), pk)).
		Select(selectNodes...).
		Get()
}
...
// doc: type=Root
func HasRoot(ctx context.Context, pk string) (bool, error) {
	...
	if v, ok := m["leafs"]; ok {
		...
		case []map[string]any: // array expansion
			...
			for _, v3 := range v2 {
				obj := new(Leaf)
				obj.unpack(v3, obj)
				o.leafs.Set(obj.PrimaryKey(), obj)
			}
		default:
			panic("Wrong type found for leafs object.")
		}
	} else {
		o.leafs.Clear()
		o.leafsIsDirty = false
	...
	err := db.WithTransaction(ctx, d, func(ctx context.Context) error {
		...
		if o.leafsIsDirty {
			...
			if oldObjs, err := QueryLeafs(ctx).
				...
				keys := o.leafs.Keys() // Make a copy of the keys, since we will change the slicemap while iterating
				for i, k := range keys {
					obj := o.leafs.Get(k)
					if obj == nil {
					...
					if obj.PrimaryKey() != k {
						...
						o.leafs.Delete(k)
						o.leafs.SetAt(i, obj.PrimaryKey(), obj)
					}
				}
			}
...
func (o *rootBase) Delete(ctx context.Context) (err error) {
	...
	broadcast.Delete(ctx, "goradd_unit", "root", o._originalPK)
	return
}
...
func (o *rootBase) encodeTo(enc db.Encoder) error {
	...
	if err := enc.Encode(&o.leafs); err != nil {
		return err
	}

	if err := enc.Encode(o.leafsIsDirty); err != nil {
		return err
	}
...
func (o *rootBase) decodeFrom(dec db.Decoder) (err error) {
	...
	if err = dec.Decode(&o.leafs); err != nil {
		return fmt.Errorf("error decoding Root.leafs: %w", err)
	}

	if err = dec.Decode(&o.leafsIsDirty); err != nil {
		return fmt.Errorf("error decoding Root.leafsIsDirty: %w", err)
	}
