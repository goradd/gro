...
type rootBase struct {
	...
	// Reverse references
	rootLeafs        maps.SliceMap[string, *Leaf] // Objects in the order they were queried
	rootLeafsIsDirty bool
...
const (
	RootIDField       = `id`
	RootNameField     = `name`
	RootRootLeafField = `rootLeafs`
)

const RootIDMaxLength = 32    // The number of runes the column can hold
...
func (o *rootBase) SetPrimaryKey(v string) {
	if o._restored {
		panic("error: Do not change a primary key for a record that has been saved. Instead, save a copy and delete the original.")
	}
	if utf8.RuneCountInString(v) > RootIDMaxLength {
		panic("attempted to set Root.Value to a value larger than its maximum length in runes")
	}
	o.idIsLoaded = true
	o.idIsDirty = true
	o.id = v
...
// RootLeaf returns a single Leaf object by primary key, if one was loaded.
// Otherwise, it will return nil. It will not return Leaf objects that are not saved.
func (o *rootBase) RootLeaf(pk string) *Leaf {
	v := o.rootLeafs.Get(pk)
	return v
}

// RootLeafs returns a slice of Leaf objects if loaded.
func (o *rootBase) RootLeafs() []*Leaf {
	return o.rootLeafs.Values()
}

// LoadRootLeafs loads a new slice of Leaf objects and returns it.
func (o *rootBase) LoadRootLeafs(ctx context.Context) ([]*Leaf, error) {
	if o.IsNew() {
		return nil, nil
	}
	for obj := range o.rootLeafs.ValuesIter() {
		if obj.IsDirty() {
			panic("You cannot load over items that have changed but have not been saved.")
		}
	}

	objs, err := LoadLeafsByRootID(ctx, o.PrimaryKey())
	if err != nil {
		return nil, err
	}
	o.rootLeafs.Clear()
	...
	for _, obj := range objs {
		pk := obj.ID()
		o.rootLeafs.Set(pk, obj)
	}

	if o.rootLeafs.Len() == 0 {
		return nil, nil
	}
	return o.rootLeafs.Values(), nil
}

// CountRootLeafs does a database query and returns the number of Leaf
// objects currently in the database that have a RootID value that equals this objects primary key.
func (o *rootBase) CountRootLeafs(ctx context.Context) (int, error) {
	return CountLeafsByRootID(ctx, o.PrimaryKey())
}

// SetRootLeafs associates the objects in objs with this Root by setting
// their RootID values to this object's primary key.
// WARNING! If it has RootLeafs already associated with it that will not be associated after a save,
// Save will panic. Be sure to delete those RootLeafs or otherwise fix those pointers before calling save.
func (o *rootBase) SetRootLeafs(objs ...*Leaf) {
	for obj := range o.rootLeafs.ValuesIter() {
		if obj.IsDirty() {
...
// The RootBuilder uses a builder pattern to create a query on the database.
// Create a RootBuilder by calling QueryRoots, which will select all
// the Root object in the database. Then filter and arrange those objects
// by calling Where, Select, etc.
// End a query by calling either Load, LoadI, LoadCursor, Get, or Count.
// A RootBuilder stores the context it will use to perform the query, and thus is
...
func newRootBuilder(ctx context.Context) *RootBuilder {
	b := RootBuilder{
		builder: query.NewBuilder(node.Root()),
		ctx:     ctx,
	}
	return &b
}
	...
	if v, ok := m["RootLeaf"]; ok {
		...
		case []map[string]any: // array expansion
			...
			for _, v3 := range v2 {
				obj := new(Leaf)
				obj.unpack(v3, obj)
				o.rootLeafs.Set(obj.PrimaryKey(), obj)
			}
		default:
			panic("Wrong type found for rootLeafs object.")
		}
	} else {
		o.rootLeafs.Clear()
		o.rootLeafsIsDirty = false
...
func (o *rootBase) update(ctx context.Context) error {
	...
	err := db.WithTransaction(ctx, d, func(ctx context.Context) error {
		...
		modifiedFields = getRootUpdateFields(o)
		...
		if o.rootLeafsIsDirty {
			...
			if oldObjs, err := QueryLeafs(ctx).
				Where(op.Equal(node.Leaf().RootID(), o.ID())).
				Select(node.Leaf().RootID()).
				Load(); err != nil {
				return err
			} else {
				for _, obj := range oldObjs {
					if !o.rootLeafs.Has(obj.PrimaryKey()) {
				...
				keys := o.rootLeafs.Keys() // Make a copy of the keys, since we will change the slicemap while iterating
				for i, k := range keys {
					obj := o.rootLeafs.Get(k)
					if obj == nil {
						// object was deleted during save?
						continue
					}
					obj.SetRootID(o.PrimaryKey())
					obj.rootIDIsDirty = true // force a change in case data is stale
					if err = obj.Save(ctx); err != nil {
						return err
					}
					if obj.PrimaryKey() != k {
						// update slice map key without changing order
						o.rootLeafs.Delete(k)
						o.rootLeafs.SetAt(i, obj.PrimaryKey(), obj)
					}
				}
			}
	...
	o.resetDirtyStatus()
	if len(modifiedFields) != 0 {
		broadcast.Update(ctx, "goradd_unit", "root", o._originalPK, anyutil.SortedKeys(modifiedFields)...)
	}
...
func (o *rootBase) insert(ctx context.Context) (err error) {
	...
	err = db.WithTransaction(ctx, d, func(context.Context) error {
		...
		insertFields = getRootInsertFields(o)
		var newPK string
...
//
// Associated RootLeaf will also be deleted since their Root fields are not nullable.
	...
	err = db.WithTransaction(ctx, d, func(context.Context) error {

		{
			objs, err := QueryLeafs(ctx).
				Where(op.Equal(node.Leaf().Root(), o.id)).
				Load()
			if err != nil {
				return err
			}
		...
		return d.Delete(ctx, "root", "id", o.id, "", 0)
	})
...
func deleteRoot(ctx context.Context, pk string) error {
	...
	err := db.WithTransaction(ctx, d, func(context.Context) error {
		if obj, err := LoadRoot(ctx,
			pk,
			node.Root().ID(),
		); err != nil {
			return err
		} else if obj == nil {
			return db.NewRecordNotFoundError("root", pk)
		} else {
			if err := obj.Delete(ctx); err != nil {
				return err
			}
...
func (o *rootBase) encodeTo(enc db.Encoder) error {
	...
	if err := enc.Encode(&o.rootLeafs); err != nil {
		return err
	}

	if err := enc.Encode(o.rootLeafsIsDirty); err != nil {
		return err
	}
...
func (o *rootBase) decodeFrom(dec db.Decoder) (err error) {
	...
	if err = dec.Decode(&o.rootLeafs); err != nil {
		return fmt.Errorf("error decoding Root.rootLeafs: %w", err)
	}

	if err = dec.Decode(&o.rootLeafsIsDirty); err != nil {
		return fmt.Errorf("error decoding Root.rootLeafsIsDirty: %w", err)
	}
