...
func (o *altLeafUnBase) LoadAltRootUn(ctx context.Context) (*AltRootUn, error) {
	...
	if o.altRootUn == nil {
		...
		// Load and cache
		o.altRootUn, err = LoadAltRootUn(ctx, o.altRootUnID)
	}
	return o.altRootUn, err
}

// SetAltRootUn sets the value of AltRootUn in the object, to be saved later using the Save() function.
func (o *altLeafUnBase) SetAltRootUn(altRootUn *AltRootUn) {
	if altRootUn == nil {
		panic("Cannot set AltRootUn to a nil value since AltRootUnID is not nullable.")
	} else {
		o.altRootUn = altRootUn
		o.altRootUnIDIsLoaded = true
		if o.altRootUnID != altRootUn.PrimaryKey() {
...
func LoadAltLeafUn(ctx context.Context, pk string, selectNodes ...query.Node) (*AltLeafUn, error) {
	return queryAltLeafUns(ctx).
		Where(op.Equal(node.AltLeafUn().ID(), pk.id)).
		Select(selectNodes...).
		Get()
}
...
// LoadAltLeafUnBy queries for a single AltLeafUn object by the given unique index values.
...
func LoadAltLeafUnBy(ctx context.Context, altRootUnID float32, selectNodes ...query.Node) (*AltLeafUn, error) {
	q := queryAltLeafUns(ctx)
	q = q.Where(op.Equal(node.AltLeafUn().AltRootUnID(), altRootUnID))
	return q.Select(selectNodes...).Get()
}

// HasAltLeafUnBy returns true if the
// given unique index values exist in the database.
// doc: type=AltLeafUn
func HasAltLeafUnBy(ctx context.Context, altRootUnID float32) (bool, error) {
	q := queryAltLeafUns(ctx)
	q = q.Where(op.Equal(node.AltLeafUn().AltRootUnID(), altRootUnID))
	v, err := q.Count()
	return v > 0, err
...
// CountAltLeafUnsBy queries the database and returns the number of AltLeafUn objects that
// have altRootUnID.
// doc: type=AltLeafUn
func CountAltLeafUnsBy(ctx context.Context, altRootUnID float32) (int, error) {
...
func (o *altLeafUnBase) unpack(m map[string]interface{}, objThis *AltLeafUn) {
	...
	if v, ok := m["alt_root_un_id"]; ok && v != nil {
		if o.altRootUnID, ok = v.(float32); ok {
			o.altRootUnIDIsLoaded = true
			o.altRootUnIDIsDirty = false
		} else {
...
func (o *altLeafUnBase) update(ctx context.Context) error {
	...
	err := db.WithTransaction(ctx, d, func(ctx context.Context) error {
		...
		if o.altRootUn != nil {
			if err := o.altRootUn.Save(ctx); err != nil {
				return err
			}
			o.SetAltRootUnID(o.altRootUn.PrimaryKey())
		}
...
func (o *altLeafUnBase) insert(ctx context.Context) (err error) {
	...
	err = db.WithTransaction(ctx, d, func(context.Context) error {
		...
		if !o.altRootUnIDIsLoaded {
			panic("a value for AltRootUnID is required, and there is no default value. Call SetAltRootUnID() before inserting the record.")
		}
...
func (o *altLeafUnBase) IsDirty() (dirty bool) {
	...
	dirty = dirty ||
		o.altRootUn != nil && o.altRootUn.IsDirty()
...
func (o *altLeafUnBase) encodeTo(enc db.Encoder) error {
	...
	if o.altRootUn == nil {
		...
		if err := enc.Encode(o.altRootUn); err != nil {
			return fmt.Errorf("error encoding AltLeafUn.altRootUn: %w", err)
		}
	}
...
func (o *altLeafUnBase) decodeFrom(dec db.Decoder) (err error) {
	...
	if err = dec.Decode(&isPtr); err != nil {
		return fmt.Errorf("error decoding AltLeafUn.altRootUn isPtr: %w", err)
	}
	if isPtr {
		if err = dec.Decode(&o.altRootUn); err != nil {
			return fmt.Errorf("error decoding AltLeafUn.altRootUn: %w", err)
		}
	}
	if err = dec.Decode(&isPtr); err != nil {
