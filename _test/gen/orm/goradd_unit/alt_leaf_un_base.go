...
func (o *altLeafUnBase) SetPrimaryKey(v string) {
	...
	if utf8.RuneCountInString(v) > AltLeafUnIDMaxLength {
		panic("attempted to set AltLeafUn.Value to a value larger than its maximum length in runes")
	}
	o.idIsLoaded = true
	o.idIsDirty = true
	o.id = v
...
// LoadAltLeafUnsByAltRootUnID queries AltLeafUn objects by the given index values.
...
func LoadAltLeafUnsByAltRootUnID(ctx context.Context, altRootUnID float32, selectNodes ...query.Node) ([]*AltLeafUn, error) {
	q := queryAltLeafUns(ctx)
	q = q.Where(op.Equal(node.AltLeafUn().AltRootUnID(), altRootUnID))
	return q.Select(selectNodes...).Load()
}

// HasAltLeafUnsByAltRootUnID returns true if the
// given index values exist in the database.
// doc: type=AltLeafUn
func HasAltLeafUnsByAltRootUnID(ctx context.Context, altRootUnID float32) (bool, error) {
	q := queryAltLeafUns(ctx)
	q = q.Where(op.Equal(node.AltLeafUn().AltRootUnID(), altRootUnID))
	v, err := q.Count()
	return v > 0, err
