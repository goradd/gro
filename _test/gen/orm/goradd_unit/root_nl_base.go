...
type rootNlBase struct {
	...
	// Reverse references
	leafNls        maps.SliceMap[string, *LeafNl] // Objects in the order they were queried
	leafNlsIsDirty bool
...
func (o *rootNlBase) LoadLeafNls(ctx context.Context) ([]*LeafNl, error) {
	...
	for obj := range o.leafNls.ValuesIter() {
		if obj.IsDirty() {
	...
	for _, obj := range objs {
		pk := obj.ID()
		o.leafNls.Set(pk, obj)
	}

	if o.leafNls.Len() == 0 {
		return nil, nil
	}
	return o.leafNls.Values(), nil
}
...
// SetLeafNls associates the objects in objs with this RootNl by setting
...
func (o *rootNlBase) SetLeafNls(objs ...*LeafNl) {
	...
	o.leafNlsIsDirty = true
}

// LoadRootNl returns a RootNl from the database.
// selectNodes lets you provide nodes for selecting specific fields or additional fields from related tables.
// See [RootNlsBuilder.Select] for more info.
func LoadRootNl(ctx context.Context, pk string, selectNodes ...query.Node) (*RootNl, error) {
	return queryRootNls(ctx).
		Where(op.Equal(node.RootNl().ID(), pk)).
		Select(selectNodes...).
		Get()
}
...
// doc: type=RootNl
func HasRootNl(ctx context.Context, pk string) (bool, error) {
	...
	if v, ok := m["leafNls"]; ok {
		...
		case []map[string]any: // array expansion
			...
			for _, v3 := range v2 {
				obj := new(LeafNl)
				obj.unpack(v3, obj)
				o.leafNls.Set(obj.PrimaryKey(), obj)
			}
		default:
			panic("Wrong type found for leafNls object.")
		}
	} else {
		o.leafNls.Clear()
		o.leafNlsIsDirty = false
		...
		if o.leafNlsIsDirty {
			...
			if currentObjs, err := QueryLeafNls(ctx).
				...
				for _, obj := range currentObjs {
					if !o.leafNls.Has(obj.PrimaryKey()) {
				...
				keys := o.leafNls.Keys() // Make a copy of the keys, since we will change the slicemap while iterating
				for i, k := range keys {
					obj := o.leafNls.Get(k)
					...
					if obj.PrimaryKey() != k {
						...
						o.leafNls.Delete(k)
						o.leafNls.SetAt(i, obj.PrimaryKey(), obj)
					}
				}
			}
...
func (o *rootNlBase) Delete(ctx context.Context) (err error) {
	...
	broadcast.Delete(ctx, "goradd_unit", "root_nl", o._originalPK)
	return
}
...
func (o *rootNlBase) Get(key string) interface{} {
	...
	case RootNlIDField:
		...
		return o.id
	case RootNlNameField:
		if !o.nameIsLoaded {
			return nil
		}
		return o.name
	case RootNlGroLockField:
		if !o.groLockIsLoaded {
			return nil
		}
		return o.groLock
	case RootNlLeafNlField:
		return o.leafNls.Values()
	}
	return nil
}
...
func (o *rootNlBase) encodeTo(enc db.Encoder) error {
	...
	if err := enc.Encode(&o.leafNls); err != nil {
		return err
	}

	if err := enc.Encode(o.leafNlsIsDirty); err != nil {
		return err
	}
...
func (o *rootNlBase) decodeFrom(dec db.Decoder) (err error) {
	...
	if err = dec.Decode(&o.leafNls); err != nil {
		return fmt.Errorf("error decoding RootNl.leafNls: %w", err)
	}

	if err = dec.Decode(&o.leafNlsIsDirty); err != nil {
		return fmt.Errorf("error decoding RootNl.leafNlsIsDirty: %w", err)
	}
