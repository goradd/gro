...
func (o *leafNBase) SetPrimaryKey(v string) {
	...
	if utf8.RuneCountInString(v) > LeafNIDMaxLength {
		panic("attempted to set LeafN.Value to a value larger than its maximum length in runes")
	}
	o.idIsLoaded = true
	o.idIsDirty = true
	o.id = v
...
// doc: type=LeafN
func HasLeafNsByRootNID(ctx context.Context, rootNID string) (bool, error) {
	q := queryLeafNs(ctx)
	q = q.Where(op.Equal(node.LeafN().RootNID(), rootNID))
	v, err := q.Count()
	return v > 0, err
