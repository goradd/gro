...
// RootNID returns the value of RootNID.
...
func (o *leafNBase) SetRootNID(v string) {
	...
	if o.objRootN != nil &&
		o.rootNID != o.objRootN.PrimaryKey() {
		o.objRootN = nil
	}
}
...
func (o *leafNBase) LoadRootN(ctx context.Context) (*RootN, error) {
	...
	if o.objRootN == nil {
		...
		// Load and cache
		o.objRootN, err = LoadRootN(ctx, o.rootNID)
	}
	return o.objRootN, err
}
...
func (o *leafNBase) SetRootN(objRootN *RootN) {
	o.rootNIDIsLoaded = true
	if objRootN == nil {
		...
		o.objRootN = objRootN
		if o.rootNIDIsNull || !o._restored || o.rootNID != objRootN.PrimaryKey() {
			o.rootNIDIsNull = false
			o.rootNID = objRootN.PrimaryKey()
			o.rootNIDIsDirty = true
		}
	}
}
...
func LoadLeafN(ctx context.Context, id string, selectNodes ...query.Node) (*LeafN, error) {
	return queryLeafNs(ctx).
		Where(op.Equal(node.LeafN().ID(), id)).
		Select(selectNodes...).
		Get()
}
...
// doc: type=LeafN
func HasLeafN(ctx context.Context, id string) (bool, error) {
...
// CountLeafNsByID queries the database and returns the number of LeafN objects that
// have id.
// doc: type=LeafN
func CountLeafNsByID(ctx context.Context, id string) (int, error) {
	return QueryLeafNs(ctx).Where(op.Equal(node.LeafN().ID(), id)).Count()
}

// CountLeafNsByName queries the database and returns the number of LeafN objects that
// have name.
// doc: type=LeafN
func CountLeafNsByName(ctx context.Context, name string) (int, error) {
	return QueryLeafNs(ctx).Where(op.Equal(node.LeafN().Name(), name)).Count()
}
...
func CountLeafNsByRootNID(ctx context.Context, rootNID string) (int, error) {
	...
	return QueryLeafNs(ctx).Where(op.Equal(node.LeafN().RootNID(), rootNID)).Count()
}

// load is the private loader that transforms data coming from the database into a tree structure reflecting the relationships
// between the object chain requested by the user in the query.
func (o *leafNBase) load(m map[string]interface{}, objThis *LeafN) {
	...
	if v, ok := m["RootN"]; ok {
		if objRootN, ok2 := v.(map[string]any); ok2 {
			o.objRootN = new(RootN)
			o.objRootN.load(objRootN, o.objRootN)
			o.rootNIDIsLoaded = true
			o.rootNIDIsDirty = false
		} else {
...
func (o *leafNBase) update(ctx context.Context) error {
	...
	err := db.ExecuteTransaction(ctx, d, func() error {
		...
		if o.objRootN != nil {
			if err := o.objRootN.Save(ctx); err != nil {
				return err
			}
			o.SetRootNID(o.objRootN.PrimaryKey())
		}
		...
		if len(modifiedFields) != 0 {
			...
			_, err2 = d.Update(ctx, "leaf_n", "id", o._originalPK, modifiedFields, "", 0)
			if err2 != nil {
				return err2
			}
		}
	...
	o.resetDirtyStatus()
	if len(modifiedFields) != 0 {
		broadcast.Update(ctx, "goradd_unit", "leaf_n", o._originalPK, all.SortedKeys(modifiedFields)...)
	}
...
func (o *leafNBase) Delete(ctx context.Context) (err error) {
	...
	d := Database()
	return d.Delete(ctx, "leaf_n", map[string]any{"ID": o.id})
	broadcast.Delete(ctx, "goradd_unit", "leaf_n", fmt.Sprint(o.id))
	return
}
...
func deleteLeafN(ctx context.Context, pk string) error {
	...
	err := d.Delete(ctx, "leaf_n", map[string]any{"ID": pk})
	if err != nil {
		return err
	}
	broadcast.Delete(ctx, "goradd_unit", "leaf_n", fmt.Sprint(pk))
	return nil
}
...
func (o *leafNBase) IsDirty() (dirty bool) {
	dirty = o.idIsDirty ||
		o.nameIsDirty ||
		o.rootNIDIsDirty ||
		(o.objRootN != nil && o.objRootN.IsDirty())
...
func (o *leafNBase) encodeTo(enc db.Encoder) error {
	...
	if o.objRootN == nil {
		...
		if err := enc.Encode(o.objRootN); err != nil {
			return fmt.Errorf("error encoding LeafN.objRootN: %w", err)
		}
	}
...
func (o *leafNBase) decodeFrom(dec db.Decoder) (err error) {
	...
	if err = dec.Decode(&isPtr); err != nil {
		return fmt.Errorf("error decoding LeafN.objRootN isPtr: %w", err)
	}
	if isPtr {
		if err = dec.Decode(&o.objRootN); err != nil {
			return fmt.Errorf("error decoding LeafN.objRootN: %w", err)
		}
	}
	if err = dec.Decode(&isPtr); err != nil {
...
func (o *leafNBase) MarshalStringMap() map[string]interface{} {
	...
	if val := o.objRootN; val != nil {
		v["rootN"] = val.MarshalStringMap()
	} else if o.rootNIDIsLoaded {
		if o.rootNIDIsNull {
			v["rootNID"] = nil
		} else {
			v["rootNID"] = o.rootNID
