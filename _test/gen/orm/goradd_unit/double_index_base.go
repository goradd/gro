...
func (o *doubleIndexBase) SetPrimaryKey(v int) {
	if o._restored {
...
func LoadDoubleIndex(ctx context.Context, pk int, selectNodes ...query.Node) (*DoubleIndex, error) {
	return queryDoubleIndices(ctx).
		Where(op.Equal(node.DoubleIndex().ID(), pk.id)).
		Select(selectNodes...).
		Get()
}
...
// LoadDoubleIndexBy queries for a single DoubleIndex object by the given unique index values.
...
func LoadDoubleIndexBy(ctx context.Context, field2Int interface{}, field2String interface{}, selectNodes ...query.Node) (*DoubleIndex, error) {
...
// HasDoubleIndexBy returns true if the
// given unique index values exist in the database.
// doc: type=DoubleIndex
func HasDoubleIndexBy(ctx context.Context, field2Int interface{}, field2String interface{}) (bool, error) {
	q := queryDoubleIndices(ctx)
	if field2Int == nil {
		q = q.Where(op.IsNull(node.DoubleIndex().Field2Int()))
	} else {
...
// See [DoubleIndicesBuilder.Select].
// If you need a more elaborate query, use QueryDoubleIndices() to start a query builder.
func LoadDoubleIndexBy(ctx context.Context, fieldInt int, fieldString string, selectNodes ...query.Node) (*DoubleIndex, error) {
	q := queryDoubleIndices(ctx)
...
// given unique index values exist in the database.
// doc: type=DoubleIndex
func HasDoubleIndexBy(ctx context.Context, fieldInt int, fieldString string) (bool, error) {
	q := queryDoubleIndices(ctx)
	q = q.Where(op.Equal(node.DoubleIndex().FieldInt(), fieldInt))
	q = q.Where(op.Equal(node.DoubleIndex().FieldString(), fieldString))
	v, err := q.Count()
...
// Create a DoubleIndexBuilder by calling QueryDoubleIndices, which will select all
...
func newDoubleIndexBuilder(ctx context.Context) *DoubleIndexBuilder {
	b := DoubleIndexBuilder{
		builder: query.NewBuilder(node.DoubleIndex()),
		ctx:     ctx,
	}
	return &b
}
...
// CountDoubleIndicesBy queries the database and returns the number of DoubleIndex objects that
// have field2Int and field2String.
// doc: type=DoubleIndex
func CountDoubleIndicesBy(ctx context.Context, field2Int int, field2String string) (int, error) {
...
// doc: type=DoubleIndex
func CountDoubleIndicesBy(ctx context.Context, fieldInt int, fieldString string) (int, error) {
	v_fieldInt := fieldInt
