...
func (o *leafUnBase) SetPrimaryKey(v string) {
	...
	if utf8.RuneCountInString(v) > LeafUnIDMaxLength {
		panic("attempted to set LeafUn.Value to a value larger than its maximum length in runes")
	}
	o.idIsLoaded = true
	o.idIsDirty = true
	o.id = v
...
// LoadLeafUnsByRootUnID queries LeafUn objects by the given index values.
...
func LoadLeafUnsByRootUnID(ctx context.Context, rootUnID string, selectNodes ...query.Node) ([]*LeafUn, error) {
	q := queryLeafUns(ctx)
	q = q.Where(op.Equal(node.LeafUn().RootUnID(), rootUnID))
	return q.Select(selectNodes...).Load()
}

// HasLeafUnsByRootUnID returns true if the
// given index values exist in the database.
// doc: type=LeafUn
func HasLeafUnsByRootUnID(ctx context.Context, rootUnID string) (bool, error) {
	q := queryLeafUns(ctx)
	q = q.Where(op.Equal(node.LeafUn().RootUnID(), rootUnID))
	v, err := q.Count()
	return v > 0, err
