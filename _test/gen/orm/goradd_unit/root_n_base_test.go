...
func TestRootN_ReferenceLoad(t *testing.T) {
	...
	obj2, err := LoadRootN(ctx, obj.PrimaryKey())
	assert.NoError(t, err)
	objPkOnly, err2 := LoadRootN(ctx, obj.PrimaryKey(), node.RootN().PrimaryKey())
	assert.NoError(t, err2)
	_ = obj2 // avoid error if there are no references
	_ = objPkOnly

	assert.Nil(t, obj2.LeafNs(), "LeafNs is not loaded initially")
	v_LeafNs, _ := obj2.LoadLeafNs(ctx)
	assert.NotNil(t, v_LeafNs)
	assert.Len(t, v_LeafNs, 1)

	// test eager loading
	obj3, _ := LoadRootN(ctx, obj.PrimaryKey(), node.RootN().LeafNs())
...
func TestRootN_ReferenceUpdateNewObjects(t *testing.T) {
	...
	obj3, _ := LoadRootN(ctx, obj2.PrimaryKey(), node.RootN().LeafNs())
	_ = obj3 // avoid error if there are no references
...
func TestRootN_Getters(t *testing.T) {
	...
	obj2, _ := LoadRootN(ctx, obj.PrimaryKey(), node.RootN().PrimaryKey())
...
func TestRootN_QueryLoad(t *testing.T) {
	...
	objs, _ := QueryRootNs(ctx).
		Where(op.Equal(node.RootN().PrimaryKey(), obj.PrimaryKey())).
		OrderBy(node.RootN().PrimaryKey()). // exercise order by
		Limit(1, 0).                        // exercise limit
		Calculation(node.RootN(), "IsTrue", op.Equal(1, 1)).
		Load()
