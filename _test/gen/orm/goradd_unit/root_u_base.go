...
type rootUBase struct {
	...
	revLeafU        *LeafU
	revLeafUIsDirty bool // is a new one being associated
...
func (o *rootUBase) LoadLeafU(ctx context.Context) (*LeafU, error) {
	if o.revLeafU != nil && o.revLeafU.IsDirty() {
		panic("The LeafU has changed. You must save it first before changing to a different one.")
	}
	var err error
	if o.revLeafU == nil {
		pk := o.ID()
		o.revLeafU, err = LoadLeafUByRootUID(ctx, pk)
	}
	return o.revLeafU, err
}

// SetLeafU associates obj with this RootU
// through the reverse relationship in LeafU.RootUID.
//
...
// The RootUBuilder uses the query.BuilderI interface to build a query.
// All query operations go through this query builder.
// End a query by calling either Load, LoadCursor, Get, Count, or Delete
type RootUBuilder interface {
	// Join(alias string, joinedTable query.Node, condition query.Node) RootUBuilder

	// Where adds a condition to filter what gets selected.
	// Calling Where multiple times will AND the conditions together.
	Where(c query.Node) RootUBuilder

	// OrderBy specifies how the resulting data should be sorted.
	// By default, the given nodes are sorted in ascending order.
	// Add Descending() to the node to specify that it should be sorted in descending order.
	OrderBy(nodes ...query.Sorter) RootUBuilder

	// Limit will return a subset of the data, limited to the offset and number of rows specified.
	// For large data sets and specific types of queries, this can be slow, because it will perform
	// the entire query before computing the limit.
	// You cannot limit a query that has selected a "many" relationship".
	Limit(maxRowCount int, offset int) RootUBuilder
	...
	Select(nodes ...query.Node) RootUBuilder
	...
	Calculation(base query.TableNodeI, alias string, operation query.OperationNodeI) RootUBuilder

	// Distinct removes duplicates from the results of the query.
	// Adding a Select() is required.
	Distinct() RootUBuilder

	// GroupBy controls how results are grouped when using aggregate functions with Calculation.
	GroupBy(nodes ...query.Node) RootUBuilder

	// Having does additional filtering on the results of the query after the query is performed.
	Having(node query.Node) RootUBuilder
...
type rootUQueryBuilder struct {
	builder *query.Builder
}

func newRootUBuilder(ctx context.Context) RootUBuilder {
	b := rootUQueryBuilder{
		builder: query.NewBuilder(ctx, node.RootU()),
	}
	return &b
}
...
// If no results come back from the query, it will return a non-nil empty slice.
func (b *rootUQueryBuilder) Load() (rootUs []*RootU, err error) {
...
// If there are any errors, nil is returned and the specific error is stored in the context.
// If no results come back from the query, it will return a non-nil empty slice.
func (b *rootUQueryBuilder) LoadI() (rootUs []query.OrmObj, err error) {
	b.builder.Command = query.BuilderCommandLoad
	database := db.GetDatabase("goradd_unit")
	var results any
	results, err = database.BuilderQuery(b.builder)
...
//
// to make sure the cursor gets closed.
func (b *rootUQueryBuilder) LoadCursor() (rootUsCursor, error) {
...
// you are selecting on one or very few items.
// If an error occurs, or no results are found, a nil is returned.
func (b *rootUQueryBuilder) Get() (*RootU, error) {
	results, err := b.Load()
...
// Where adds a condition to filter what gets selected.
// Calling Where multiple times will AND the conditions together.
func (b *rootUQueryBuilder) Where(c query.Node) RootUBuilder {
	b.builder.Where(c)
	return b
}
...
// By default, the given nodes are sorted in ascending order.
// Add Descending() to the node to specify that it should be sorted in descending order.
func (b *rootUQueryBuilder) OrderBy(nodes ...query.Sorter) RootUBuilder {
...
func (b *rootUQueryBuilder) Limit(maxRowCount int, offset int) RootUBuilder {
...
func (b *rootUQueryBuilder) Select(nodes ...query.Node) RootUBuilder {
...
func (b *rootUQueryBuilder) Calculation(base query.TableNodeI, alias string, operation query.OperationNodeI) RootUBuilder {
...
func (b *rootUQueryBuilder) Distinct() RootUBuilder {
	b.builder.Distinct()
	return b
}

// GroupBy controls how results are grouped when using aggregate functions with Calculation.
func (b *rootUQueryBuilder) GroupBy(nodes ...query.Node) RootUBuilder {
	b.builder.GroupBy(nodes...)
	return b
}

// Having does additional filtering on the results of the query after the query is performed.
func (b *rootUQueryBuilder) Having(node query.Node) RootUBuilder {
...
// To not count NULL values, use Where in the builder with a NotNull operation.
// To count distinct combinations of items, call Distinct() on the builder.
func (b *rootUQueryBuilder) Count() (int, error) {
...
func (o *rootUBase) load(m map[string]interface{}, objThis *RootU) {
	...
	if v, ok := m["LeafU"]; ok {
		if v2, ok2 := v.(map[string]any); ok2 {
			o.revLeafU = new(LeafU)
			o.revLeafU.load(v2, o.revLeafU)
			o.revLeafUIsDirty = false
		} else {
			panic("Wrong type found for rootUID object.")
		}
	} else {
		o.revLeafU = nil
		o.revLeafUIsDirty = false
...
func (o *rootUBase) update(ctx context.Context) error {
	...
	err := db.ExecuteTransaction(ctx, d, func() error {
		...
		if o.revLeafUIsDirty {
			...
			if oldObj, err := QueryLeafUs(ctx).
				Where(op.Equal(node.LeafU().RootUID(), o.PrimaryKey())).
				Select(node.LeafU().RootUID()).
				Get(); err != nil {
				return err
			} else if oldObj != nil {
				if o.revLeafU == nil || oldObj.PrimaryKey() != o.revLeafU.PrimaryKey() {
			...
			if o.revLeafU != nil {
				o.revLeafU.SetRootUID(o.PrimaryKey())
				if err := o.revLeafU.Save(ctx); err != nil {
					return err
				}
			}
	...
	o.resetDirtyStatus()
	if len(modifiedFields) != 0 {
		broadcast.Update(ctx, "goradd_unit", "root_u", o._originalPK, all.SortedKeys(modifiedFields)...)
	}
...
//
// An associated {= rev.ReverseIdentifier()  will also be deleted since its RootUID field is not nullable.
func (o *rootUBase) Delete(ctx context.Context) (err error) {
	...
	err = db.ExecuteTransaction(ctx, d, func() error {
		...
		return d.Delete(ctx, "root_u", map[string]any{"ID": o.id})
	})
...
func deleteRootU(ctx context.Context, pk string) error {
	if obj, err := LoadRootU(ctx, pk, node.RootU().PrimaryKey()); err != nil {
		return err
	} else if obj != nil {
		if err := obj.Delete(ctx); err != nil {
			return err
...
func (o *rootUBase) IsDirty() (dirty bool) {
	...
	dirty = dirty || (o.revLeafU != nil && o.revLeafU.IsDirty())
...
func (o *rootUBase) encodeTo(enc db.Encoder) error {
	...
	if o.revLeafU == nil {
		...
		if err := enc.Encode(o.revLeafU); err != nil {
			return fmt.Errorf("error encoding RootU.revLeafU: %w", err)
		}
	}

	if err := enc.Encode(o.revLeafUIsDirty); err != nil {
		return fmt.Errorf("error encoding RootU.revLeafUIsDirty: %w", err)
	}
...
func (o *rootUBase) decodeFrom(dec db.Decoder) (err error) {
	...
	if err = dec.Decode(&isPtr); err != nil {
		return fmt.Errorf("error decoding RootU.revLeafU isPtr: %w", err)
	}
	if isPtr {
		if err = dec.Decode(&o.revLeafU); err != nil {
			return fmt.Errorf("error decoding RootU.revLeafU: %w", err)
		}
	}

	if err = dec.Decode(&o.revLeafUIsDirty); err != nil {
		return fmt.Errorf("error decoding RootU.revLeafUIsDirty: %w", err)
	}
	if err = dec.Decode(&isPtr); err != nil {
