...
func TestSerializeReverseReferencesRootNTable(t *testing.T) {
	{
		...
		assert.True(t, query.NodesMatch(RootN().RootNLeafNs().ID(), n2.(LeafNNode).ID()))
		assert.True(t, query.NodesMatch(RootN().RootNLeafNs().Name(), n2.(LeafNNode).Name()))
		assert.True(t, query.NodesMatch(RootN().RootNLeafNs().RootNID(), n2.(LeafNNode).RootNID()))
		assert.True(t, query.NodesMatch(RootN().RootNLeafNs().RootN(), n2.(LeafNNode).RootN()))
