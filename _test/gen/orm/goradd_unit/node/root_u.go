...
type RootUNode interface {
	...
	// LeafU represents the LeafU reference to a LeafU object.
	LeafU() LeafUNode
}
...
// LeafU represents the one-to-one relationship formed by the reverse reference from the
// root_u_id column in the leaf_u table.
func (n rootUTable) LeafU() LeafUNode {
	cn := &leafUReverse{
		ReverseNode: query.ReverseNode{
			ColumnQueryName: "root_u_id",
			Identifier:      "LeafU",
			ReceiverType:    query.ColTypeString,
			IsUnique:        true,
		},
...
func (n *rootUReference) LeafU() LeafUNode {
	cn := n.rootUTable.LeafU().(*leafUReverse)
	query.NodeSetParent(cn, n)
	return cn
}
