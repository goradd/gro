...
type RootNlNode interface {
	...
	// RootNlLeafNl represents the RootNlLeafNl reverse reference to LeafNl objects
	// through the RootNlID foreign key there.
	RootNlLeafNls() LeafNlNode
}
...
// RootNlLeafNl represents the many-to-one relationship formed by the reverse reference from the
// root_nl_id column in the leaf_nl table.
func (n rootNlTable) RootNlLeafNls() LeafNlNode {
	cn := &leafNlReverse{
		ReverseNode: query.ReverseNode{
			ForeignKey: "root_nl_id",
			Identifier: "RootNlLeafNls",
			IsUnique:   false,
		},
	}
	query.NodeSetParent(cn, n)
...
func (n *rootNlReference) RootNlLeafNls() LeafNlNode {
	cn := n.rootNlTable.RootNlLeafNls().(*leafNlReverse)
	query.NodeSetParent(cn, n)
	return cn
}
