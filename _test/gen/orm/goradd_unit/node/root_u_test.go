...
func TestSerializeReverseReferencesRootUTable(t *testing.T) {
	{
		...
		assert.True(t, query.NodesMatch(RootU().RootULeafUs().ID(), n2.(LeafUNode).ID()))
		assert.True(t, query.NodesMatch(RootU().RootULeafUs().Name(), n2.(LeafUNode).Name()))
		assert.True(t, query.NodesMatch(RootU().RootULeafUs().RootUID(), n2.(LeafUNode).RootUID()))
		assert.True(t, query.NodesMatch(RootU().RootULeafUs().RootU(), n2.(LeafUNode).RootU()))
