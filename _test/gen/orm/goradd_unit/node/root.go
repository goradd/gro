...
type RootNode interface {
	...
	// RootLeaf represents the RootLeaf reverse reference to Leaf objects
...
// RootLeaf represents the many-to-one relationship formed by the reverse reference from the
// root_id column in the leaf table.
func (n rootTable) RootLeafs() LeafNode {
	cn := &leafReverse{
		ReverseNode: query.ReverseNode{
			ForeignKey: "root_id",
			Identifier: "RootLeafs",
			IsUnique:   false,
		},
	}
	query.NodeSetParent(cn, n)
...
func (n *rootReference) RootLeafs() LeafNode {
	cn := n.rootTable.RootLeafs().(*leafReverse)
	query.NodeSetParent(cn, n)
	return cn
}
