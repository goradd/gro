...
func TestSerializeReverseReferencesRootUnTable(t *testing.T) {

	{
		...
		assert.True(t, query.NodesMatch(RootUn().LeafUn().ID(), n2.(LeafUnNode).ID()))
		assert.True(t, query.NodesMatch(RootUn().LeafUn().Name(), n2.(LeafUnNode).Name()))
		assert.True(t, query.NodesMatch(RootUn().LeafUn().RootUnID(), n2.(LeafUnNode).RootUnID()))
		assert.True(t, query.NodesMatch(RootUn().LeafUn().RootUn(), n2.(LeafUnNode).RootUn()))
