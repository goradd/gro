...
type RootLNode interface {
	...
	// RootLLeafL represents the RootLLeafL reverse reference to LeafL objects
	// through the RootLID foreign key there.
	RootLLeafLs() LeafLNode
}
...
func (n rootLTable) PrimaryKeys() []*query.ColumnNode {
...
func (n *rootLReference) PrimaryKeys() []*query.ColumnNode {
	return []*query.ColumnNode{
		n.ID(),
	}
}
...
// RootLLeafL represents the many-to-one relationship formed by the reverse reference from the
// root_l_id column in the leaf_l table.
func (n rootLTable) RootLLeafLs() LeafLNode {
	cn := &leafLReverse{
		ReverseNode: query.ReverseNode{
			ForeignKey: "root_l_id",
			Identifier: "RootLLeafLs",
			IsUnique:   false,
		},
	}
	query.NodeSetParent(cn, n)
...
func (n *rootLReference) RootLLeafLs() LeafLNode {
	cn := n.rootLTable.RootLLeafLs().(*leafLReverse)
	query.NodeSetParent(cn, n)
	return cn
}
