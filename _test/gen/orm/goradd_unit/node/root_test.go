...
func TestSerializeReverseReferencesRootTable(t *testing.T) {
	{
		...
		assert.True(t, query.NodesMatch(Root().RootLeafs().ID(), n2.(LeafNode).ID()))
		assert.True(t, query.NodesMatch(Root().RootLeafs().Name(), n2.(LeafNode).Name()))
		assert.True(t, query.NodesMatch(Root().RootLeafs().RootID(), n2.(LeafNode).RootID()))
		assert.True(t, query.NodesMatch(Root().RootLeafs().Root(), n2.(LeafNode).Root()))
