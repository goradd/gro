...
type RootUnNode interface {
	...
	// RootUnLeafUn represents the RootUnLeafUn reverse reference to LeafUn objects
	// through the RootUnID foreign key there.
	RootUnLeafUns() LeafUnNode
}
...
// RootUnLeafUn represents the many-to-one relationship formed by the reverse reference from the
// root_un_id column in the leaf_un table.
func (n rootUnTable) RootUnLeafUns() LeafUnNode {
	cn := &leafUnReverse{
		ReverseNode: query.ReverseNode{
			ForeignKey: "root_un_id",
			Identifier: "RootUnLeafUns",
			IsUnique:   false,
		},
	}
	query.NodeSetParent(cn, n)
...
func (n *rootUnReference) RootUnLeafUns() LeafUnNode {
	cn := n.rootUnTable.RootUnLeafUns().(*leafUnReverse)
	query.NodeSetParent(cn, n)
	return cn
}
