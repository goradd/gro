...
func TestSerializeReverseReferencesRootUlTable(t *testing.T) {
	{
		...
		assert.True(t, query.NodesMatch(RootUl().RootUlLeafUls().ID(), n2.(LeafUlNode).ID()))
		assert.True(t, query.NodesMatch(RootUl().RootUlLeafUls().Name(), n2.(LeafUlNode).Name()))
		assert.True(t, query.NodesMatch(RootUl().RootUlLeafUls().GroLock(), n2.(LeafUlNode).GroLock()))
		assert.True(t, query.NodesMatch(RootUl().RootUlLeafUls().RootUlID(), n2.(LeafUlNode).RootUlID()))
		assert.True(t, query.NodesMatch(RootUl().RootUlLeafUls().RootUl(), n2.(LeafUlNode).RootUl()))
