...
func TestDbJson(t *testing.T) {
	...
	v_RootL, _ := QueryRootLs(ctx).OrderBy(node.RootL().PrimaryKey()).Get()                               // gets first record
	v_RootN, _ := QueryRootNs(ctx).OrderBy(node.RootN().PrimaryKey()).Get()                               // gets first record
	v_RootNl, _ := QueryRootNls(ctx).OrderBy(node.RootNl().PrimaryKey()).Get()                            // gets first record
	v_RootU, _ := QueryRootUs(ctx).OrderBy(node.RootU().PrimaryKey()).Get()                               // gets first record
	v_RootUl, _ := QueryRootUls(ctx).OrderBy(node.RootUl().PrimaryKey()).Get()                            // gets first record
	v_RootUn, _ := QueryRootUns(ctx).OrderBy(node.RootUn().PrimaryKey()).Get()                            // gets first record
	v_RootUnl, _ := QueryRootUnls(ctx).OrderBy(node.RootUnl().PrimaryKey()).Get()                         // gets first record
	...
	v_LeafNl, _ := QueryLeafNls(ctx).OrderBy(node.LeafNl().PrimaryKey()).Get()                            // gets first record
	v_LeafU, _ := QueryLeafUs(ctx).OrderBy(node.LeafU().PrimaryKey()).Get()                               // gets first record
	...
	if v_RootL != nil {
		obj, _ := QueryRootLs(ctx).OrderBy(node.RootL().PrimaryKey()).Get()
		assertEqualFieldsRootL(t, v_RootL, obj)
	}
	if v_RootN != nil {
		obj, _ := QueryRootNs(ctx).OrderBy(node.RootN().PrimaryKey()).Get()
		assertEqualFieldsRootN(t, v_RootN, obj)
	}
	if v_RootNl != nil {
		obj, _ := QueryRootNls(ctx).OrderBy(node.RootNl().PrimaryKey()).Get()
		assertEqualFieldsRootNl(t, v_RootNl, obj)
	}
	if v_RootU != nil {
		obj, _ := QueryRootUs(ctx).OrderBy(node.RootU().PrimaryKey()).Get()
		assertEqualFieldsRootU(t, v_RootU, obj)
	}
	if v_RootUl != nil {
		obj, _ := QueryRootUls(ctx).OrderBy(node.RootUl().PrimaryKey()).Get()
		assertEqualFieldsRootUl(t, v_RootUl, obj)
	}
	if v_RootUn != nil {
		obj, _ := QueryRootUns(ctx).OrderBy(node.RootUn().PrimaryKey()).Get()
		assertEqualFieldsRootUn(t, v_RootUn, obj)
	}
	if v_RootUnl != nil {
		obj, _ := QueryRootUnls(ctx).OrderBy(node.RootUnl().PrimaryKey()).Get()
	...
	if v_LeafL != nil {
		obj, _ := QueryLeafLs(ctx).OrderBy(node.LeafL().PrimaryKey()).Get()
		assertEqualFieldsLeafL(t, v_LeafL, obj)
	}
	if v_LeafN != nil {
		obj, _ := QueryLeafNs(ctx).OrderBy(node.LeafN().PrimaryKey()).Get()
		assertEqualFieldsLeafN(t, v_LeafN, obj)
	}
	if v_LeafNl != nil {
		obj, _ := QueryLeafNls(ctx).OrderBy(node.LeafNl().PrimaryKey()).Get()
		assertEqualFieldsLeafNl(t, v_LeafNl, obj)
	}
	if v_LeafU != nil {
		obj, _ := QueryLeafUs(ctx).OrderBy(node.LeafU().PrimaryKey()).Get()
		assertEqualFieldsLeafU(t, v_LeafU, obj)
	}
	if v_LeafUl != nil {
		obj, _ := QueryLeafUls(ctx).OrderBy(node.LeafUl().PrimaryKey()).Get()
		assertEqualFieldsLeafUl(t, v_LeafUl, obj)
	}
	if v_LeafUn != nil {
		obj, _ := QueryLeafUns(ctx).OrderBy(node.LeafUn().PrimaryKey()).Get()
