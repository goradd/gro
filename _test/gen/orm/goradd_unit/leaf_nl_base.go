...
func (o *leafNlBase) SetPrimaryKey(v string) {
	...
	if utf8.RuneCountInString(v) > LeafNlIDMaxLength {
		panic("attempted to set LeafNl.Value to a value larger than its maximum length in runes")
	}
	o.idIsLoaded = true
	o.idIsDirty = true
	o.id = v
...
// SetRootNl sets the value of RootNl in the object, to be saved later using the Save() function.
func (o *leafNlBase) SetRootNl(rootNl *RootNl) {
	if rootNl == nil {
		panic("Cannot set RootNl to a nil value since RootNlID is not nullable.")
	} else {
		o.rootNl = rootNl
		o.rootNlIDIsLoaded = true
		if o.rootNlID != rootNl.PrimaryKey() {
...
func LoadLeafNlsByRootNlID(ctx context.Context, rootNlID string, selectNodes ...query.Node) ([]*LeafNl, error) {
	q := queryLeafNls(ctx)
...
// doc: type=LeafNl
func HasLeafNlsByRootNlID(ctx context.Context, rootNlID string) (bool, error) {
	q := queryLeafNls(ctx)
	q = q.Where(op.Equal(node.LeafNl().RootNlID(), rootNlID))
	v, err := q.Count()
	return v > 0, err
...
func (o *leafNlBase) unpack(m map[string]interface{}, objThis *LeafNl) {
	...
	if v, ok := m["root_nl_id"]; ok && v != nil {
		if o.rootNlID, ok = v.(string); ok {
			o.rootNlIDIsLoaded = true
			o.rootNlIDIsDirty = false
		} else {
...
func (o *leafNlBase) insert(ctx context.Context) (err error) {
	...
	err = db.WithTransaction(ctx, d, func(context.Context) error {
		...
		if !o.rootNlIDIsLoaded {
			panic("a value for RootNlID is required, and there is no default value. Call SetRootNlID() before inserting the record.")
		}
