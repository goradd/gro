...
func LoadLeafNl(ctx context.Context, pk string, selectNodes ...query.Node) (*LeafNl, error) {
	return queryLeafNls(ctx).
		Where(op.Equal(node.LeafNl().ID(), pk.id)).
		Select(selectNodes...).
		Get()
}
...
// LoadLeafNlsBy queries LeafNl objects by the given index values.
...
func LoadLeafNlsBy(ctx context.Context, rootNlID string, selectNodes ...query.Node) ([]*LeafNl, error) {
	q := queryLeafNls(ctx)
	q = q.Where(op.Equal(node.LeafNl().RootNlID(), rootNlID))
	return q.Select(selectNodes...).Load()
}

// HasLeafNlsBy returns true if the
// given index values exist in the database.
// doc: type=LeafNl
func HasLeafNlsBy(ctx context.Context, rootNlID string) (bool, error) {
	q := queryLeafNls(ctx)
	q = q.Where(op.Equal(node.LeafNl().RootNlID(), rootNlID))
	v, err := q.Count()
	return v > 0, err
...
// CountLeafNlsBy queries the database and returns the number of LeafNl objects that
// have rootNlID.
// doc: type=LeafNl
func CountLeafNlsBy(ctx context.Context, rootNlID string) (int, error) {
	v_rootNlID := rootNlID
	return QueryLeafNls(ctx).
