...
func updateMaximalSampleRoot(ctx context.Context, obj *Root) {
	...
	obj.SetRootLeafs(createMinimalSampleLeaf())
}
...
func deleteSampleRoot(ctx context.Context, obj *Root) {
	...
	for _, item := range obj.RootLeafs() {
...
func TestRoot_ReferenceLoad(t *testing.T) {
	...
	assert.Nil(t, obj2.RootLeafs(), "RootLeaf is not loaded initially")
	v_RootLeafs, _ := obj2.LoadRootLeafs(ctx)
	assert.NotNil(t, v_RootLeafs)
	assert.Len(t, v_RootLeafs, 1)

	// test eager loading
	obj3, _ := LoadRoot(ctx, obj.PrimaryKey(), node.Root().ID(),
		node.Root().Name(),
		node.Root().RootLeafs(),
	)
	_ = obj3 // avoid error if there are no references

	assert.Equal(t, len(obj2.RootLeafs()), len(obj3.RootLeafs()))
...
func TestRoot_ReferenceUpdateNewObjects(t *testing.T) {
	...
	obj3, _ := LoadRoot(ctx, obj2.PrimaryKey(), node.Root().RootLeafs())
	_ = obj3 // avoid error if there are no references
...
func TestRoot_ReferenceUpdateOldObjects(t *testing.T) {
	...
	updateMinimalSampleLeaf(obj.RootLeafs()[0])

	assert.NoError(t, obj.Save(ctx))

	obj2, _ := LoadRoot(ctx, obj.PrimaryKey(), node.Root().ID(),
		node.Root().Name(),
		node.Root().RootLeafs(),
	)
	_ = obj2 // avoid error if there are no references

	assertEqualFieldsLeaf(t, obj2.RootLeafs()[0], obj.RootLeafs()[0])
