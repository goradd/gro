...
func (o *leafUlBase) SetPrimaryKey(v string) {
	...
	if utf8.RuneCountInString(v) > LeafUlIDMaxLength {
		panic("attempted to set LeafUl.Value to a value larger than its maximum length in runes")
	}
	o.idIsLoaded = true
	o.idIsDirty = true
	o.id = v
...
// LoadLeafUlsByRootUlID queries LeafUl objects by the given index values.
...
func LoadLeafUlsByRootUlID(ctx context.Context, rootUlID string, selectNodes ...query.Node) ([]*LeafUl, error) {
	q := queryLeafUls(ctx)
	q = q.Where(op.Equal(node.LeafUl().RootUlID(), rootUlID))
	return q.Select(selectNodes...).Load()
}

// HasLeafUlsByRootUlID returns true if the
// given index values exist in the database.
// doc: type=LeafUl
func HasLeafUlsByRootUlID(ctx context.Context, rootUlID string) (bool, error) {
	q := queryLeafUls(ctx)
	q = q.Where(op.Equal(node.LeafUl().RootUlID(), rootUlID))
	v, err := q.Count()
	return v > 0, err
