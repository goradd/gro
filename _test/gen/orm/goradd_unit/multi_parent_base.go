...
func (o *multiParentBase) SetPrimaryKey(v string) {
	...
	if utf8.RuneCountInString(v) > MultiParentIDMaxLength {
		panic("attempted to set MultiParent.Value to a value larger than its maximum length in runes")
	}
	o.idIsLoaded = true
	o.idIsDirty = true
	o.id = v
...
func LoadMultiParentsByParent1ID(ctx context.Context, parent1ID string, selectNodes ...query.Node) ([]*MultiParent, error) {
...
// doc: type=MultiParent
func HasMultiParentsByParent1ID(ctx context.Context, parent1ID string) (bool, error) {
	q := queryMultiParents(ctx)
	q = q.Where(op.Equal(node.MultiParent().Parent1ID(), parent1ID))
	v, err := q.Count()
	return v > 0, err
...
// See [MultiParentsBuilder.Select].
// If you need a more elaborate query, use QueryMultiParents() to start a query builder.
func LoadMultiParentsByParent2ID(ctx context.Context, parent2ID string, selectNodes ...query.Node) ([]*MultiParent, error) {
	q := queryMultiParents(ctx)
...
// given index values exist in the database.
// doc: type=MultiParent
func HasMultiParentsByParent2ID(ctx context.Context, parent2ID string) (bool, error) {
	q := queryMultiParents(ctx)
	q = q.Where(op.Equal(node.MultiParent().Parent2ID(), parent2ID))
	v, err := q.Count()
	return v > 0, err
