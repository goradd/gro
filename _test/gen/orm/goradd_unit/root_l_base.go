...
type rootLBase struct {
	...
	// Reverse references
	rootLLeafLs        maps.SliceMap[string, *LeafL] // Objects in the order they were queried
	rootLLeafLsIsDirty bool
...
const (
	...
	RootLRootLLeafLField = `rootLLeafLs`
)
...
func (o *rootLBase) SetPrimaryKey(v string) {
	...
	if utf8.RuneCountInString(v) > RootLIDMaxLength {
		panic("attempted to set RootL.Value to a value larger than its maximum length in runes")
	}
	o.idIsLoaded = true
	o.idIsDirty = true
	o.id = v
...
// RootLLeafL returns a single LeafL object by primary key, if one was loaded.
// Otherwise, it will return nil. It will not return LeafL objects that are not saved.
func (o *rootLBase) RootLLeafL(pk string) *LeafL {
	v := o.rootLLeafLs.Get(pk)
	return v
}

// RootLLeafLs returns a slice of LeafL objects if loaded.
func (o *rootLBase) RootLLeafLs() []*LeafL {
	return o.rootLLeafLs.Values()
}

// LoadRootLLeafLs loads a new slice of LeafL objects and returns it.
func (o *rootLBase) LoadRootLLeafLs(ctx context.Context) ([]*LeafL, error) {
	if o.IsNew() {
		return nil, nil
	}
	for obj := range o.rootLLeafLs.ValuesIter() {
		if obj.IsDirty() {
	...
	for _, obj := range objs {
		pk := obj.ID()
		o.rootLLeafLs.Set(pk, obj)
	}

	if o.rootLLeafLs.Len() == 0 {
		return nil, nil
	}
	return o.rootLLeafLs.Values(), nil
}

// CountRootLLeafLs does a database query and returns the number of LeafL
// objects currently in the database that have a RootLID value that equals this objects primary key.
func (o *rootLBase) CountRootLLeafLs(ctx context.Context) (int, error) {
	return CountLeafLsByRootLID(ctx, o.PrimaryKey())
}

// SetRootLLeafLs associates the objects in objs with this RootL by setting
// their RootLID values to this object's primary key.
// WARNING! If it has RootLLeafLs already associated with it that will not be associated after a save,
// Save will panic. Be sure to delete those RootLLeafLs or otherwise fix those pointers before calling save.
func (o *rootLBase) SetRootLLeafLs(objs ...*LeafL) {
	for obj := range o.rootLLeafLs.ValuesIter() {
		if obj.IsDirty() {
	...
	if v, ok := m["RootLLeafL"]; ok {
		...
		case []map[string]any: // array expansion
			...
			for _, v3 := range v2 {
				obj := new(LeafL)
				obj.unpack(v3, obj)
				o.rootLLeafLs.Set(obj.PrimaryKey(), obj)
			}
		default:
			panic("Wrong type found for rootLLeafLs object.")
		}
	} else {
		o.rootLLeafLs.Clear()
		o.rootLLeafLsIsDirty = false
		...
		if o.rootLLeafLsIsDirty {
			...
			if oldObjs, err := QueryLeafLs(ctx).
				...
				for _, obj := range oldObjs {
					if !o.rootLLeafLs.Has(obj.PrimaryKey()) {
				...
				keys := o.rootLLeafLs.Keys() // Make a copy of the keys, since we will change the slicemap while iterating
				for i, k := range keys {
					obj := o.rootLLeafLs.Get(k)
					if obj == nil {
					...
					if obj.PrimaryKey() != k {
						...
						o.rootLLeafLs.Delete(k)
						o.rootLLeafLs.SetAt(i, obj.PrimaryKey(), obj)
					}
				}
			}
...
//
// Associated RootLLeafL will also be deleted since their RootL fields are not nullable.
	...
	err = db.WithTransaction(ctx, d, func(context.Context) error {

		{
			objs, err := QueryLeafLs(ctx).
				Where(op.Equal(node.LeafL().RootL(), o.id)).
				Load()
			if err != nil {
				return err
			}
...
func (o *rootLBase) encodeTo(enc db.Encoder) error {
	...
	if err := enc.Encode(&o.rootLLeafLs); err != nil {
		return err
	}

	if err := enc.Encode(o.rootLLeafLsIsDirty); err != nil {
		return err
	}
...
func (o *rootLBase) decodeFrom(dec db.Decoder) (err error) {
	...
	if err = dec.Decode(&o.rootLLeafLs); err != nil {
		return fmt.Errorf("error decoding RootL.rootLLeafLs: %w", err)
	}

	if err = dec.Decode(&o.rootLLeafLsIsDirty); err != nil {
		return fmt.Errorf("error decoding RootL.rootLLeafLsIsDirty: %w", err)
	}
