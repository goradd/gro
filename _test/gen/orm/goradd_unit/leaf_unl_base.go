...
func (o *leafUnlBase) SetPrimaryKey(v string) {
	...
	if utf8.RuneCountInString(v) > LeafUnlIDMaxLength {
		panic("attempted to set LeafUnl.Value to a value larger than its maximum length in runes")
	}
	o.idIsLoaded = true
	o.idIsDirty = true
	o.id = v
...
// LoadLeafUnlsByRootUnlID queries LeafUnl objects by the given index values.
...
func LoadLeafUnlsByRootUnlID(ctx context.Context, rootUnlID string, selectNodes ...query.Node) ([]*LeafUnl, error) {
	q := queryLeafUnls(ctx)
	q = q.Where(op.Equal(node.LeafUnl().RootUnlID(), rootUnlID))
	return q.Select(selectNodes...).Load()
}

// HasLeafUnlsByRootUnlID returns true if the
// given index values exist in the database.
// doc: type=LeafUnl
func HasLeafUnlsByRootUnlID(ctx context.Context, rootUnlID string) (bool, error) {
	q := queryLeafUnls(ctx)
	q = q.Where(op.Equal(node.LeafUnl().RootUnlID(), rootUnlID))
	v, err := q.Count()
	return v > 0, err
