...
func (o *leafUnlBase) SetRootUnlID(v string) {
	...
	if o.rootUnl != nil &&
		o.rootUnlID != o.rootUnl.PrimaryKey() {
		o.rootUnl = nil
	}
}
...
func (o *leafUnlBase) LoadRootUnl(ctx context.Context) (*RootUnl, error) {
	...
	if o.rootUnl == nil {
		...
		// Load and cache
		o.rootUnl, err = LoadRootUnl(ctx, o.rootUnlID)
	}
	return o.rootUnl, err
}

// SetRootUnl sets the value of RootUnl in the object, to be saved later using the Save() function.
func (o *leafUnlBase) SetRootUnl(rootUnl *RootUnl) {
	if rootUnl == nil {
		panic("Cannot set RootUnl to a nil value since RootUnlID is not nullable.")
	} else {
		o.rootUnl = rootUnl
		o.rootUnlIDIsLoaded = true
		if o.rootUnlID != rootUnl.PrimaryKey() {
...
func LoadLeafUnl(ctx context.Context, pk string, selectNodes ...query.Node) (*LeafUnl, error) {
	return queryLeafUnls(ctx).
		Where(op.Equal(node.LeafUnl().ID(), pk.id)).
		Select(selectNodes...).
		Get()
}
...
// LoadLeafUnlBy queries for a single LeafUnl object by the given unique index values.
...
func LoadLeafUnlBy(ctx context.Context, rootUnlID string, selectNodes ...query.Node) (*LeafUnl, error) {
	q := queryLeafUnls(ctx)
	q = q.Where(op.Equal(node.LeafUnl().RootUnlID(), rootUnlID))
	return q.Select(selectNodes...).Get()
}

// HasLeafUnlBy returns true if the
// given unique index values exist in the database.
// doc: type=LeafUnl
func HasLeafUnlBy(ctx context.Context, rootUnlID string) (bool, error) {
	q := queryLeafUnls(ctx)
	q = q.Where(op.Equal(node.LeafUnl().RootUnlID(), rootUnlID))
	v, err := q.Count()
	return v > 0, err
...
// CountLeafUnlsBy queries the database and returns the number of LeafUnl objects that
// have rootUnlID.
// doc: type=LeafUnl
func CountLeafUnlsBy(ctx context.Context, rootUnlID string) (int, error) {
...
func (o *leafUnlBase) unpack(m map[string]interface{}, objThis *LeafUnl) {
	...
	if v, ok := m["root_unl_id"]; ok && v != nil {
		if o.rootUnlID, ok = v.(string); ok {
			o.rootUnlIDIsLoaded = true
			o.rootUnlIDIsDirty = false
		} else {
	...
	if v, ok := m["RootUnl"]; ok {
		if rootUnl, ok2 := v.(map[string]any); ok2 {
			o.rootUnl = new(RootUnl)
			o.rootUnl.unpack(rootUnl, o.rootUnl)
			o.rootUnlIDIsLoaded = true
			o.rootUnlIDIsDirty = false
		} else {
...
func (o *leafUnlBase) update(ctx context.Context) error {
	...
	err := db.WithTransaction(ctx, d, func(ctx context.Context) error {
		...
		if o.rootUnl != nil {
			if err := o.rootUnl.Save(ctx); err != nil {
				return err
			}
			o.SetRootUnlID(o.rootUnl.PrimaryKey())
		}
...
func (o *leafUnlBase) insert(ctx context.Context) (err error) {
	...
	err = db.WithTransaction(ctx, d, func(context.Context) error {
		...
		if !o.rootUnlIDIsLoaded {
			panic("a value for RootUnlID is required, and there is no default value. Call SetRootUnlID() before inserting the record.")
		}
...
func (o *leafUnlBase) IsDirty() (dirty bool) {
	...
	dirty = dirty ||
		o.rootUnl != nil && o.rootUnl.IsDirty()
...
func (o *leafUnlBase) encodeTo(enc db.Encoder) error {
	...
	if o.rootUnl == nil {
		...
		if err := enc.Encode(o.rootUnl); err != nil {
			return fmt.Errorf("error encoding LeafUnl.rootUnl: %w", err)
		}
	}
...
func (o *leafUnlBase) decodeFrom(dec db.Decoder) (err error) {
	...
	if err = dec.Decode(&isPtr); err != nil {
		return fmt.Errorf("error decoding LeafUnl.rootUnl isPtr: %w", err)
	}
	if isPtr {
		if err = dec.Decode(&o.rootUnl); err != nil {
			return fmt.Errorf("error decoding LeafUnl.rootUnl: %w", err)
		}
	}
	if err = dec.Decode(&isPtr); err != nil {
