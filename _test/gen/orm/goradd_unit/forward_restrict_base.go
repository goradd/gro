...
// To not count NULL values, use Where in the builder with a NotNull operation.
// To count distinct combinations of items, call Distinct() on the builder.
func (b *forwardRestrictQueryBuilder) Count() int {
...
func CountForwardRestrictByID(ctx context.Context, id string) int {
	return queryForwardRestricts(ctx).Where(op.Equal(node.ForwardRestrict().ID(), id)).Count()
}
...
func CountForwardRestrictByName(ctx context.Context, name string) int {
	return queryForwardRestricts(ctx).Where(op.Equal(node.ForwardRestrict().Name(), name)).Count()
}
...
func CountForwardRestrictByReverseID(ctx context.Context, reverseID string) int {
	...
	return queryForwardRestricts(ctx).Where(op.Equal(node.ForwardRestrict().ReverseID(), reverseID)).Count()
}
...
func (o *forwardRestrictBase) Save(ctx context.Context) error {
	if o._restored {
		return o.update(ctx)
	} else {
		return o.insert(ctx)
...
func (o *forwardRestrictBase) update(ctx context.Context) error {
	if !o._restored {
	...
	err := db.ExecuteTransaction(ctx, d, func() error {
		...
		if len(modifiedFields) != 0 {
			err := d.Update(ctx, "forward_restrict", "id", o._originalPK, modifiedFields, "", 0)
			if err != nil {
				return err
			}
		}
...
func (o *forwardRestrictBase) insert(ctx context.Context) (err error) {
	d := Database()
	err = db.ExecuteTransaction(ctx, d, func() error {
...
func (o *forwardRestrictBase) Delete(ctx context.Context) (err error) {
	if !o._restored {
...
func deleteForwardRestrict(ctx context.Context, pk string) error {
