	...
	db := Database()
	{
		...
		cursor, err := db.Query(ctx, "leaf_nl_assn",
			map[string]query.ReceiverType{
				"leaf_nl_id": query.ColTypeString,
				"leaf_nl_id": query.ColTypeString,
			},
			nil,
			nil)
...
func jsonDecodeLeafNlAssn(ctx context.Context, decoder *json.Decoder) error {
	...
	for decoder.More() {
		var imp struct {
			Src  string `json:"leaf_nl_id"`
			Dest string `json:"leaf_nl_id"`
		}

		if err = decoder.Decode(&imp); err != nil {
			return err
		}
		db.Associate(ctx, database, "leaf_nl_assn", "leaf_nl_id", imp.Src, "leaf_nl_id", imp.Dest)
	}
