...
type reverseBase struct {
	...
	revForwardCascadeUniqueIsDirty bool // is a new one being associated
	...
	revForwardNullUniqueIsDirty bool // is a new one being associated
	...
	revForwardRestrictUniqueIsDirty bool // is a new one being associated
...
// See [ReversesBuilder.Select] for more info.
func LoadReverse(ctx context.Context, id string, selectNodes ...query.Node) *Reverse {
...
func (b *reverseQueryBuilder) Join(alias string, joinedTable query.Node, condition query.Node) ReverseBuilder {
...
// By default, all the columns of the reverse table will be queried and loaded.
// If nodes contains columns from the reverse table, that will limit the columns queried and loaded to only those columns.
...
// To not count NULL values, use Where in the builder with a NotNull operation.
// To count distinct combinations of items, call Distinct() on the builder.
func (b *reverseQueryBuilder) Count() int {
...
func CountReverseByID(ctx context.Context, id string) int {
	return queryReverses(ctx).Where(op.Equal(node.Reverse().ID(), id)).Count()
}
...
func CountReverseByName(ctx context.Context, name string) int {
	return queryReverses(ctx).Where(op.Equal(node.Reverse().Name(), name)).Count()
}
...
func (o *reverseBase) update(ctx context.Context) error {
	if !o._restored {
	...
	err := db.ExecuteTransaction(ctx, d, func() error {
		...
		if len(modifiedFields) != 0 {
			err := d.Update(ctx, "reverse", "id", o._originalPK, modifiedFields, "", 0)
			if err != nil {
				return err
			}
		}
		...
		if o.revForwardCascadesIsDirty {
			...
			currentObjs := QueryForwardCascades(ctx).
		...
		if o.revForwardNullsIsDirty {
			...
			currentObjs := QueryForwardNulls(ctx).
		...
		if o.revForwardRestrictsIsDirty {
			...
			oldObjs := QueryForwardRestricts(ctx).
