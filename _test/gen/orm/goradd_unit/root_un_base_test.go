...
func updateMaximalSampleRootUn(ctx context.Context, obj *RootUn) {
	...
	obj.SetRootUnLeafUns(createMinimalSampleLeafUn())
}
...
func deleteSampleRootUn(ctx context.Context, obj *RootUn) {
	...
	for _, item := range obj.RootUnLeafUns() {
		deleteSampleLeafUn(ctx, item)
	}
...
func TestRootUn_ReferenceLoad(t *testing.T) {
	...
	assert.Nil(t, obj2.RootUnLeafUns(), "RootUnLeafUn is not loaded initially")
	v_RootUnLeafUns, _ := obj2.LoadRootUnLeafUns(ctx)
	assert.NotNil(t, v_RootUnLeafUns)
	assert.Len(t, v_RootUnLeafUns, 1)

	// test eager loading
	obj3, _ := LoadRootUn(ctx, obj.PrimaryKey(), node.RootUn().ID(),
		node.RootUn().Name(),
		node.RootUn().RootUnLeafUns(),
	)
	_ = obj3 // avoid error if there are no references

	assert.Equal(t, len(obj2.RootUnLeafUns()), len(obj3.RootUnLeafUns()))
...
func TestRootUn_ReferenceUpdateNewObjects(t *testing.T) {
	...
	obj3, _ := LoadRootUn(ctx, obj2.PrimaryKey(), node.RootUn().RootUnLeafUns())
	_ = obj3 // avoid error if there are no references
...
func TestRootUn_ReferenceUpdateOldObjects(t *testing.T) {
	...
	updateMinimalSampleLeafUn(obj.RootUnLeafUns()[0])

	assert.NoError(t, obj.Save(ctx))

	obj2, _ := LoadRootUn(ctx, obj.PrimaryKey(), node.RootUn().ID(),
		node.RootUn().Name(),
		node.RootUn().RootUnLeafUns(),
	)
	_ = obj2 // avoid error if there are no references

	assertEqualFieldsLeafUn(t, obj2.RootUnLeafUns()[0], obj.RootUnLeafUns()[0])
