...
type rootUnlBase struct {
	...
	// Reverse references
	rootUnlLeafUnls        maps.SliceMap[string, *LeafUnl] // Objects in the order they were queried
	rootUnlLeafUnlsIsDirty bool
...
const (
	...
	RootUnlRootUnlLeafUnlField = `rootUnlLeafUnls`
)
...
func (o *rootUnlBase) Initialize() {
	...
	o.rootUnlLeafUnls.Clear()
	o.rootUnlLeafUnlsIsDirty = false
...
func (o *rootUnlBase) SetPrimaryKey(v string) {
	if o._restored {
		panic("error: Do not change a primary key for a record that has been saved. Instead, save a copy and delete the original.")
	}
	if utf8.RuneCountInString(v) > RootUnlIDMaxLength {
		panic("attempted to set RootUnl.Value to a value larger than its maximum length in runes")
	}
	o.idIsLoaded = true
	o.idIsDirty = true
	o.id = v
...
// RootUnlLeafUnl returns a single LeafUnl object by primary key, if one was loaded.
// Otherwise, it will return nil. It will not return LeafUnl objects that are not saved.
func (o *rootUnlBase) RootUnlLeafUnl(pk string) *LeafUnl {
	v := o.rootUnlLeafUnls.Get(pk)
	return v
}

// RootUnlLeafUnls returns a slice of LeafUnl objects if loaded.
func (o *rootUnlBase) RootUnlLeafUnls() []*LeafUnl {
	return o.rootUnlLeafUnls.Values()
}

// LoadRootUnlLeafUnls loads a new slice of LeafUnl objects and returns it.
func (o *rootUnlBase) LoadRootUnlLeafUnls(ctx context.Context) ([]*LeafUnl, error) {
	if o.IsNew() {
		return nil, nil
	}
	for obj := range o.rootUnlLeafUnls.ValuesIter() {
		if obj.IsDirty() {
			panic("You cannot load over items that have changed but have not been saved.")
		}
	}

	objs, err := LoadLeafUnlsByRootUnlID(ctx, o.PrimaryKey())
	if err != nil {
		return nil, err
	}
	o.rootUnlLeafUnls.Clear()
	...
	for _, obj := range objs {
		pk := obj.ID()
		o.rootUnlLeafUnls.Set(pk, obj)
	}

	if o.rootUnlLeafUnls.Len() == 0 {
		return nil, nil
	}
	return o.rootUnlLeafUnls.Values(), nil
}

// CountRootUnlLeafUnls does a database query and returns the number of LeafUnl
// objects currently in the database that have a RootUnlID value that equals this objects primary key.
func (o *rootUnlBase) CountRootUnlLeafUnls(ctx context.Context) (int, error) {
	return CountLeafUnlsByRootUnlID(ctx, o.PrimaryKey())
}

// SetRootUnlLeafUnls associates the objects in objs with this RootUnl by setting
// their RootUnlID values to this object's primary key.
// WARNING! If it has RootUnlLeafUnls already associated with it that will not be associated after a save,
// Save will panic. Be sure to delete those RootUnlLeafUnls or otherwise fix those pointers before calling save.
func (o *rootUnlBase) SetRootUnlLeafUnls(objs ...*LeafUnl) {
	for obj := range o.rootUnlLeafUnls.ValuesIter() {
		if obj.IsDirty() {
	...
	if v, ok := m["RootUnlLeafUnl"]; ok {
		...
		case []map[string]any: // array expansion
			...
			for _, v3 := range v2 {
				obj := new(LeafUnl)
				obj.unpack(v3, obj)
				o.rootUnlLeafUnls.Set(obj.PrimaryKey(), obj)
			}
		default:
			panic("Wrong type found for rootUnlLeafUnls object.")
		}
	} else {
		o.rootUnlLeafUnls.Clear()
		o.rootUnlLeafUnlsIsDirty = false
...
func (o *rootUnlBase) update(ctx context.Context) error {
	...
	err := db.WithTransaction(ctx, d, func(ctx context.Context) error {
		...
		if o.rootUnlLeafUnlsIsDirty {
			...
			if oldObjs, err := QueryLeafUnls(ctx).
				...
				for _, obj := range oldObjs {
					if !o.rootUnlLeafUnls.Has(obj.PrimaryKey()) {
				...
				keys := o.rootUnlLeafUnls.Keys() // Make a copy of the keys, since we will change the slicemap while iterating
				for i, k := range keys {
					obj := o.rootUnlLeafUnls.Get(k)
					if obj == nil {
						// object was deleted during save?
						continue
					}
					obj.SetRootUnlID(o.PrimaryKey())
					obj.rootUnlIDIsDirty = true // force a change in case data is stale
					if err = obj.Save(ctx); err != nil {
						return err
					}
					if obj.PrimaryKey() != k {
						// update slice map key without changing order
						o.rootUnlLeafUnls.Delete(k)
						o.rootUnlLeafUnls.SetAt(i, obj.PrimaryKey(), obj)
					}
				}
			}
...
func (o *rootUnlBase) insert(ctx context.Context) (err error) {
	...
	err = db.WithTransaction(ctx, d, func(context.Context) error {
		...
		if o.rootUnlLeafUnls.Len() > 0 {
			keys := o.rootUnlLeafUnls.Keys()
			for i, k := range keys {
				obj := o.rootUnlLeafUnls.Get(k)
				obj.SetRootUnlID(newPK)
				if err = obj.Save(ctx); err != nil {
					return err
				}
				if obj.PrimaryKey() != k {
...
//
// Associated RootUnlLeafUnl will also be deleted since their RootUnl fields are not nullable.
	...
	err = db.WithTransaction(ctx, d, func(context.Context) error {

		{
			objs, err := QueryLeafUnls(ctx).
				Where(op.Equal(node.LeafUnl().RootUnl(), o.id)).
				Load()
			if err != nil {
				return err
			}
...
func (o *rootUnlBase) encodeTo(enc db.Encoder) error {
	...
	if err := enc.Encode(&o.rootUnlLeafUnls); err != nil {
		return err
	}

	if err := enc.Encode(o.rootUnlLeafUnlsIsDirty); err != nil {
		return err
	}
...
func (o *rootUnlBase) decodeFrom(dec db.Decoder) (err error) {
	...
	if err = dec.Decode(&o.rootUnlLeafUnls); err != nil {
		return fmt.Errorf("error decoding RootUnl.rootUnlLeafUnls: %w", err)
	}

	if err = dec.Decode(&o.rootUnlLeafUnlsIsDirty); err != nil {
		return fmt.Errorf("error decoding RootUnl.rootUnlLeafUnlsIsDirty: %w", err)
	}
