...
const (
	AutoGen_ID           = `ID`
	AutoGen_GroLock      = `GroLock`
	AutoGen_GroTimestamp = `GroTimestamp`
	AutoGen_Created      = `Created`
	AutoGen_Modified     = `Modified`
	AutoGen_Name         = `Name`
)
...
func LoadAutoGen(ctx context.Context, id string, selectNodes ...query.Node) (*AutoGen, error) {
	return queryAutoGens(ctx).
		Where(op.Equal(node.AutoGen().ID(), id)).
		Select(selectNodes...).
		Get()
}
...
// doc: type=AutoGen
func HasAutoGen(ctx context.Context, id string) (bool, error) {
...
// Start a query by calling QueryAutoGens, which will select all
...
type AutoGenBuilder interface {
	...
	Select(nodes ...query.Node) AutoGenBuilder
	...
	Calculation(base query.TableNodeI, alias string, operation query.OperationNodeI) AutoGenBuilder

	// Distinct removes duplicates from the results of the query.
	// Adding a Select() is required when using Distinct.
	Distinct() AutoGenBuilder

	// GroupBy controls how results are grouped when using aggregate functions with Calculation.
	GroupBy(nodes ...query.Node) AutoGenBuilder

	// Having does additional filtering on the results of the query after the query is performed.
	Having(node query.Node) AutoGenBuilder
	...
	LoadCursor() (autoGensCursor, error)
...
type autoGenQueryBuilder struct {
	builder *query.Builder
}

func newAutoGenBuilder(ctx context.Context) AutoGenBuilder {
	b := autoGenQueryBuilder{
		builder: query.NewBuilder(ctx, node.AutoGen()),
	}
	return &b
}
...
// If no results come back from the query, it will return a non-nil empty slice.
func (b *autoGenQueryBuilder) Load() (autoGens []*AutoGen, err error) {
...
// If there are any errors, nil is returned and the specific error is stored in the context.
// If no results come back from the query, it will return a non-nil empty slice.
func (b *autoGenQueryBuilder) LoadI() (autoGens []query.OrmObj, err error) {
	b.builder.Command = query.BuilderCommandLoad
	database := db.GetDatabase("goradd_unit")
	var results any
	results, err = database.BuilderQuery(b.builder)
...
//
// to make sure the cursor gets closed.
func (b *autoGenQueryBuilder) LoadCursor() (autoGensCursor, error) {
...
// you are selecting on one or very few items.
// If an error occurs, or no results are found, a nil is returned.
func (b *autoGenQueryBuilder) Get() (*AutoGen, error) {
	results, err := b.Load()
...
// Where adds a condition to filter what gets selected.
// Calling Where multiple times will AND the conditions together.
func (b *autoGenQueryBuilder) Where(c query.Node) AutoGenBuilder {
	b.builder.Where(c)
	return b
}
...
// By default, the given nodes are sorted in ascending order.
// Add Descending() to the node to specify that it should be sorted in descending order.
func (b *autoGenQueryBuilder) OrderBy(nodes ...query.Sorter) AutoGenBuilder {
...
func (b *autoGenQueryBuilder) Limit(maxRowCount int, offset int) AutoGenBuilder {
...
func (b *autoGenQueryBuilder) Select(nodes ...query.Node) AutoGenBuilder {
...
func (b *autoGenQueryBuilder) Calculation(base query.TableNodeI, alias string, operation query.OperationNodeI) AutoGenBuilder {
...
func (b *autoGenQueryBuilder) Distinct() AutoGenBuilder {
	b.builder.Distinct()
	return b
}

// GroupBy controls how results are grouped when using aggregate functions with Calculation.
func (b *autoGenQueryBuilder) GroupBy(nodes ...query.Node) AutoGenBuilder {
	b.builder.GroupBy(nodes...)
	return b
}

// Having does additional filtering on the results of the query after the query is performed.
func (b *autoGenQueryBuilder) Having(node query.Node) AutoGenBuilder {
...
// To not count NULL values, use Where in the builder with a NotNull operation.
// To count distinct combinations of items, call Distinct() on the builder.
func (b *autoGenQueryBuilder) Count() (int, error) {
...
func (o *autoGenBase) update(ctx context.Context) error {
	...
	err := db.ExecuteTransaction(ctx, d, func() error {
		...
		if len(modifiedFields) != 0 {
			...
			newLock, err2 = d.Update(ctx, "auto_gen", "id", o._originalPK, modifiedFields, "gro_lock", o.GroLock())
			if err2 != nil {
				return err2
			}
		}
...
func (o *autoGenBase) Delete(ctx context.Context) (err error) {
	...
	d := Database()
	err = d.Delete(ctx, "auto_gen", "ID", o.id, "gro_lock", o.GroLock())
	if err != nil {
		return err
	}
	broadcast.Delete(ctx, "goradd_unit", "auto_gen", fmt.Sprint(o.id))
	return
}
...
func deleteAutoGen(ctx context.Context, pk string) error {
	...
	err := d.Delete(ctx, "auto_gen", "ID", pk, "", 0)
	if err != nil {
		return err
	}
	broadcast.Delete(ctx, "goradd_unit", "auto_gen", fmt.Sprint(pk))
	return err
}
