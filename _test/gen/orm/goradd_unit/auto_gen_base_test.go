...
func TestAutoGen_ReferenceLoad(t *testing.T) {
	...
	obj2, err := LoadAutoGen(ctx, obj.PrimaryKey())
	assert.NoError(t, err)
	objPkOnly, err2 := LoadAutoGen(ctx, obj.PrimaryKey(), node.AutoGen().PrimaryKey())
...
func TestAutoGen_Getters(t *testing.T) {
	...
	obj2, _ := LoadAutoGen(ctx, obj.PrimaryKey(), node.AutoGen().PrimaryKey())

	assert.Equal(t, obj.ID(), obj.Get(node.AutoGen().ID().Identifier))
	assert.Equal(t, obj.GroLock(), obj.Get(node.AutoGen().GroLock().Identifier))
	assert.Panics(t, func() { obj2.GroLock() })
	assert.Nil(t, obj2.Get(node.AutoGen().GroLock().Identifier))
	assert.Equal(t, obj.GroTimestamp(), obj.Get(node.AutoGen().GroTimestamp().Identifier))
	assert.Panics(t, func() { obj2.GroTimestamp() })
	assert.Nil(t, obj2.Get(node.AutoGen().GroTimestamp().Identifier))
	assert.Equal(t, obj.Created(), obj.Get(node.AutoGen().Created().Identifier))
	assert.Panics(t, func() { obj2.Created() })
	assert.Nil(t, obj2.Get(node.AutoGen().Created().Identifier))
	assert.Equal(t, obj.Modified(), obj.Get(node.AutoGen().Modified().Identifier))
	assert.Panics(t, func() { obj2.Modified() })
	assert.Nil(t, obj2.Get(node.AutoGen().Modified().Identifier))
	assert.Equal(t, obj.Name(), obj.Get(node.AutoGen().Name().Identifier))
	assert.Panics(t, func() { obj2.Name() })
	assert.Nil(t, obj2.Get(node.AutoGen().Name().Identifier))
}
...
func TestAutoGen_QueryLoad(t *testing.T) {
	...
	objs, _ := QueryAutoGens(ctx).
		Where(op.Equal(node.AutoGen().PrimaryKey(), obj.PrimaryKey())).
		OrderBy(node.AutoGen().PrimaryKey()). // exercise order by
		Limit(1, 0).                          // exercise limit
		Calculation(node.AutoGen(), "IsTrue", op.Equal(1, 1)).
		Load()
