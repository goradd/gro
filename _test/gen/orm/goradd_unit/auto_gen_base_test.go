...
func TestAutoGen_ReferenceLoad(t *testing.T) {
	...
	// test eager loading
	obj3, _ := LoadAutoGen(ctx, obj.PrimaryKey(), node.AutoGen().ID(),
		node.AutoGen().GroLock(),
		node.AutoGen().GroTimestamp(),
		node.AutoGen().Created(),
		node.AutoGen().Modified(),
		node.AutoGen().Name(),
	)
...
func TestAutoGen_ReferenceUpdateOldObjects(t *testing.T) {
	...
	obj2, _ := LoadAutoGen(ctx, obj.PrimaryKey(), node.AutoGen().ID(),
		node.AutoGen().GroLock(),
		node.AutoGen().GroTimestamp(),
		node.AutoGen().Created(),
		node.AutoGen().Modified(),
