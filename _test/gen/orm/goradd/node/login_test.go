...
func TestSerializeReferencesLoginTable(t *testing.T) {
	{
		...
		assert.True(t, query.NodesMatch(Login().Person().ManagerProject(), n2.(PersonNode).ManagerProject()))
		assert.True(t, query.NodesMatch(Login().Person().PersonAddress(), n2.(PersonNode).PersonAddress()))
