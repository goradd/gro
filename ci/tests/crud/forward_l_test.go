...
func TestForwardLock(t *testing.T) {
	...
	// Insert-insert
	l := goradd_unit2.NewLeafL()
	r := goradd_unit2.NewRootL()
	...
	var l2 *goradd_unit2.LeafL
	l2, err = goradd_unit2.LoadLeafL(ctx, l.ID(), node2.LeafL().RootL())
	require.NoError(t, err)
	...
	// Insert-update
	l3 := goradd_unit2.NewLeafL()
	l3.SetName("leafForwardLock3")
	r.SetName("rootForwardLock3")
	l3.SetRootL(r)
	err = l3.Save(ctx)
	require.NoError(t, err)
	l2, err = goradd_unit2.LoadLeafL(ctx, l3.ID(), node2.LeafL().RootL())
	require.NoError(t, err)
...
func TestForwardLockCollision(t *testing.T) {
	...
	// 2nd level
	l2, _ = goradd_unit2.LoadLeafL(ctx, l.ID(), node2.LeafL().RootL())
...
func TestForwardLockTwo(t *testing.T) {
	...
	l2 := goradd_unit2.NewLeafL()
	l2.SetName("leafForwardLockTwo2")
	l2.SetRootL(r)
	require.NoError(t, l2.Save(ctx))

	r2, err := goradd_unit2.LoadRootL(ctx, r.ID(), node2.RootL().LeafLs())
	require.NoError(t, err)
...
func TestForwardLockDelete(t *testing.T) {
	...
	l2, err = goradd_unit2.LoadLeafL(ctx, l.ID())
	assert.NoError(t, err)
	require.NotNil(t, l2)
