...
func TestForwardUniqueNullable(t *testing.T) {
	...
	// Insert-insert
	l1 := goradd_unit2.NewLeafUn()
	r1 := goradd_unit2.NewRootUn()
	...
	// loading back
	l1b, err := goradd_unit2.LoadLeafUn(ctx, l1.ID(), node.LeafUn().RootUn())
...
func TestForwardUniqueNullableCollision(t *testing.T) {
	...
	var l2 *goradd_unit2.LeafUn
	l2, err = goradd_unit2.LoadLeafUn(ctx, l.ID(), node.LeafUn().RootUn())
	require.NoError(t, err)
	...
	l3, err3 := goradd_unit2.LoadLeafUn(ctx, l.ID(), node.LeafUn().RootUn())
...
func TestForwardUniqueNullableDelete(t *testing.T) {
	...
	r2, err := goradd_unit2.LoadRootUn(ctx, r.ID())
	require.NoError(t, err)
	assert.NotNil(t, r2)
}
