...
func TestReverseNullableLock(t *testing.T) {
	...
	var r2 *goradd_unit2.RootNl
	r2, err = goradd_unit2.LoadRootNl(ctx, r.ID(), node.RootNl().LeafNls())
	require.NoError(t, err)
	...
	// Insert-update
	r3 := goradd_unit2.NewRootNl()
	r3.SetName("rootReverseNullableLock3")
	l.SetName("leafReverseNullableLock3")
	r3.SetLeafNls(l)
	err = r3.Save(ctx)
	require.NoError(t, err)
	r2, err = goradd_unit2.LoadRootNl(ctx, r3.ID(), node.RootNl().LeafNls())
	require.NoError(t, err)
...
func TestReverseNullableLockCollision(t *testing.T) {
	...
	// Level 2
	r2, _ = goradd_unit2.LoadRootNl(ctx, r.ID(), node.RootNl().LeafNls())
