...
func TestForwardNullableLock(t *testing.T) {
	...
	// Insert-insert
	l := goradd_unit2.NewLeafNl()
	r := goradd_unit2.NewRootNl()
	...
	var l2 *goradd_unit2.LeafNl
	l2, err = goradd_unit2.LoadLeafNl(ctx, l.ID(), node2.LeafNl().RootNl())
	require.NoError(t, err)
	...
	// Insert-update
	l3 := goradd_unit2.NewLeafNl()
	l3.SetName("leafForwardNullableLock3")
	r.SetName("rootForwardNullableLock3")
	l3.SetRootNl(r)
	err = l3.Save(ctx)
	require.NoError(t, err)
	l2, err = goradd_unit2.LoadLeafNl(ctx, l3.ID(), node2.LeafNl().RootNl())
	require.NoError(t, err)
...
func TestForwardNullableLockCollision(t *testing.T) {
	...
	// 2nd level
	l2, _ = goradd_unit2.LoadLeafNl(ctx, l.ID(), node2.LeafNl().RootNl())
...
func TestForwardNullableLockTwo(t *testing.T) {
	ctx := context.Background()
	defer goradd_unit2.ClearAll(ctx)
	l := goradd_unit2.NewLeafNl()
	r := goradd_unit2.NewRootNl()
	...
	l2 := goradd_unit2.NewLeafNl()
	l2.SetName("leafForwardNullableLockTwo2")
	l2.SetRootNl(r)
	require.NoError(t, l2.Save(ctx))

	r2, err := goradd_unit2.LoadRootNl(ctx, r.ID(), node2.RootNl().LeafNls())
	require.NoError(t, err)
...
func TestForwardNullableLockDelete(t *testing.T) {
	...
	// Collision on Change
	l2, err := goradd_unit2.LoadLeafNl(ctx, l.ID())
	require.NoError(t, err)
