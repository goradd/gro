...
func TestReverse(t *testing.T) {
	...
	var r2 *goradd_unit2.Root
	r2, err = goradd_unit2.LoadRoot(ctx, r.ID(), node.Root().Leafs())
	require.NoError(t, err)
	...
	// Insert-update
	r3 := goradd_unit2.NewRoot()
	r3.SetName("rootReverse3")
	l.SetName("leafReverse3")
	r3.SetLeafs(l)
	err = r3.Save(ctx)
	require.NoError(t, err)
	r2, err = goradd_unit2.LoadRoot(ctx, r3.ID(), node.Root().Leafs())
	require.NoError(t, err)
...
func TestReverseCollision(t *testing.T) {
	...
	r3, err3 := goradd_unit2.LoadRoot(ctx, r.ID(), node.Root().Leafs())
