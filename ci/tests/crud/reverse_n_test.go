...
func TestReverseNullable(t *testing.T) {
	...
	var r2 *goradd_unit2.RootN
	r2, err = goradd_unit2.LoadRootN(ctx, r.ID(), node.RootN().LeafNs())
	require.NoError(t, err)
	...
	// Insert-update
	r3 := goradd_unit2.NewRootN()
	r3.SetName("rootReverseNullable3")
	l.SetName("leafReverseNullable3")
	r3.SetLeafNs(l)
	err = r3.Save(ctx)
	require.NoError(t, err)
	r2, err = goradd_unit2.LoadRootN(ctx, r3.ID(), node.RootN().LeafNs())
	require.NoError(t, err)
...
func TestReverseNullableCollision(t *testing.T) {
	...
	r3, err3 := goradd_unit2.LoadRootN(ctx, r.ID(), node.RootN().LeafNs())
