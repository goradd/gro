...
func TestReverseUnique(t *testing.T) {
	...
	var r2 *goradd_unit2.RootU
	r2, err = goradd_unit2.LoadRootU(ctx, r.ID(), node.RootU().LeafU())
	require.NoError(t, err)
	...
	err = r3.Save(ctx)
	require.NoError(t, err)
	r2, err = goradd_unit2.LoadRootU(ctx, r3.ID(), node.RootU().LeafU())
	require.NoError(t, err)
...
func TestReverseUniqueCollision(t *testing.T) {
	...
	r3, err3 := goradd_unit2.LoadRootU(ctx, r.ID(), node.RootU().LeafU())
