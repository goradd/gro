...
func TestForward(t *testing.T) {
	...
	// Insert-insert
	l := goradd_unit2.NewLeaf()
	r := goradd_unit2.NewRoot()
	...
	var l2 *goradd_unit2.Leaf
	l2, err = goradd_unit2.LoadLeaf(ctx, l.ID(), node2.Leaf().Root())
	require.NoError(t, err)
	...
	// Insert-update
	l3 := goradd_unit2.NewLeaf()
	l3.SetName("leafForward3")
	r.SetName("rootForward3")
	l3.SetRoot(r)
	err = l3.Save(ctx)
	require.NoError(t, err)
	l2, err = goradd_unit2.LoadLeaf(ctx, l3.ID(), node2.Leaf().Root())
	require.NoError(t, err)
...
func TestForwardCollision(t *testing.T) {
	...
	l3, err3 := goradd_unit2.LoadLeaf(ctx, l.ID(), node2.Leaf().Root())
...
func TestForwardTwo(t *testing.T) {
	...
	l2 := goradd_unit2.NewLeaf()
	l2.SetName("leafForwardTwo2")
	l2.SetRoot(r)
	require.NoError(t, l2.Save(ctx))

	r2, err := goradd_unit2.LoadRoot(ctx, r.ID(), node2.Root().Leafs())
	require.NoError(t, err)
...
func TestForwardDelete(t *testing.T) {
	...
	l2, err := goradd_unit2.LoadLeaf(ctx, l.ID())
	require.NoError(t, err)
	assert.Nil(t, l2)

	r2, err := goradd_unit2.LoadRoot(ctx, r.ID())
	require.NoError(t, err)
	assert.NotNil(t, r2)
}
