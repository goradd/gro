...
func TestAssociationLock(t *testing.T) {
	...
	var l3 *goradd_unit2.LeafNl
	l3, err = goradd_unit2.LoadLeafNl(ctx, l1.ID(), node.LeafNl().Leaf2s())
	require.NoError(t, err)
	...
	// Insert-update
	l4 := goradd_unit2.NewLeafNl()
	l4.SetName("leafAssociationLock4")
	l3.SetName("leafAssociationLock111")
	l4.SetLeaf2s(l3)
	err = l4.Save(ctx)
	require.NoError(t, err)
	l3, err = goradd_unit2.LoadLeafNl(ctx, l4.ID(), node.LeafNl().Leaf2s())
	require.NoError(t, err)
...
func TestAssociationLockCollision(t *testing.T) {
	...
	// 2nd level
	l3, _ = goradd_unit2.LoadLeafNl(ctx, l1.ID(), node.LeafNl().Leaf2s())
...
func TestAssociationLockTwo(t *testing.T) {
	...
	l4, err := goradd_unit2.LoadLeafNl(ctx, l1.ID(), node.LeafNl().Leaf2s())
	require.NoError(t, err)
