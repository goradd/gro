...
func TestReverseLock(t *testing.T) {
	...
	var r2 *goradd_unit2.RootL
	r2, err = goradd_unit2.LoadRootL(ctx, r.ID(), node.RootL().LeafLs())
	require.NoError(t, err)
	...
	// Insert-update
	r3 := goradd_unit2.NewRootL()
	r3.SetName("rootReverseLock3")
	l.SetName("leafReverseLock3")
	r3.SetLeafLs(l)
	err = r3.Save(ctx)
	require.NoError(t, err)
	r2, err = goradd_unit2.LoadRootL(ctx, r3.ID(), node.RootL().LeafLs())
	require.NoError(t, err)
...
func TestReverseLockCollision(t *testing.T) {
	...
	// Level 2
	r2, _ = goradd_unit2.LoadRootL(ctx, r.ID(), node.RootL().LeafLs())
